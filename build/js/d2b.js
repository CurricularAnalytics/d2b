...
(function () {
	...
	d2b.CONSTANTS = {
		...
		DEFAULTHEIGHT: function DEFAULTHEIGHT() {
			return 540;
		},
		DEFAULTMARGIN: function DEFAULTMARGIN() {
			return { left: 0, right: 0, top: 0, bottom: 0 };
		},
		DEFAULTFORCEDMARGIN: function DEFAULTFORCEDMARGIN() {
		...
		DEFAULTEVENTS: function DEFAULTEVENTS() {
			return {
				elementMouseover: function elementMouseover() {},
				elementMouseout: function elementMouseout() {},
				elementClick: function elementClick() {}
			};
		},
		ANIMATIONLENGTHS: function ANIMATIONLENGTHS() {
			return { normal: 500, short: 200, long: 1000 };
		}
	};
  ...
  * pie chart generator
  */
	d2b.chart.pie = function () {

		var model = d2b.model.chart(update);
		var $$ = model.values();
		var chart = model.base();

		// configure legend
		$$.legend.active(true).clickable(true).dblclickable(true).on('mouseover.d2b-pie', function (d) {
			return d3.select(d.elem).each(arcMouseover);
		}).on('mouseout.d2b-pie', function (d) {
			return d3.select(d.elem).each(arcMouseout);
		}).on('click.d2b-pie', chart.update).on('dblclick.d2b-pie', chart.update);
		...
		model.addProp('donutRatio', 0).addProp('startAngle', 0, null, function (d) {
			...
			$$.legend.key(d);
			pie.key(d);
		}).addProp('tooltip', d2b.tooltip().followMouse(true).html(function (d) {
			return "<b>" + $$.label(d.data) + ":</b> " + $$.value(d.data);
		})).addPropFunctor('center', null).addPropFunctor('radius', null).addPropFunctor('sort', null).addPropFunctor('color', function (d) {
		...
		function update() {
			...
			var total = d3.sum(data, function (d, i) {
				return $$.value.call(chart, d, i);
			});

			// Select and enter pie chart 'g' element.
			var chartGroup = $$.svg.main.selectAll('.d2b-pie-chart').data([data]);
			var newChartGroup = chartGroup.enter().append('g').attr('class', 'd2b-pie-chart');

			// Compute radius.
			var radius = getRadius();

			// Transition the d2b pie chart.
			var transition = chartGroup.datum(function (d) {
				d = layout(d);
				...
				return d;
			}).transition().duration($$.duration).call(pie);
			...
			var arcGroup = chartGroup.selectAll('.d2b-pie-arc').each(function (d) {
				this.transitioning = true;
				d.data.elem = this;
			}).on('mouseover.d2b-pie', arcMouseover).on('mouseout.d2b-pie', arcMouseout).call($$.tooltip);

			// Enter arc percent label if it doesn't already exist.
			var arcPercent = arcGroup.selectAll('.d2b-pie-arc-percent').data(function (d) {
				return [d];
			});
			arcPercent.enter().append('g').attr('class', 'd2b-pie-arc-percent').append('text').attr('y', 6).text('test');
			...
			arcGroup.each(function (d) {
				...
				var current = elem.select('.d2b-pie-arc-path').node().current;
				var percentGroup = elem.select('.d2b-pie-arc-percent');
				percentGroup.node().current = current;
				var percentText = percentGroup.select('text');
				var percentTextNode = percentText.node();
			...
			var arcUpdate = transition.selectAll('.d2b-pie-arc').each('end', function (d) {
				this.transitioning = false;
			}).select('.d2b-pie-arc-percent').call(d2b.centroidTween, arc).select('text').styleTween('opacity', function (d) {
				var _this = this;

				return function (t) {
					return _this.current > 0.03 ? 1 : 0;
				};
			}).call(d2b.numberTween, function (d) {
				return d.value / total;
			}, percent);

			// Position the pie chart.
			var coords = chartCoords(radius);
			newChartGroup.attr('transform', "translate(" + coords.x + ", " + coords.y + ")");
			chartGroup.transition().duration($$.duration).attr('transform', "translate(" + coords.x + ", " + coords.y + ")");
		}
		...
		function chartCoords(radius) {
			var coords = $$.center.call(chart, $$.width, $$.height, radius);
		...
		function getRadius() {
			return $$.radius.call(chart, $$.width, $$.height) || Math.min($$.width, $$.height) / 2;
		}
  ...
  */
	d2b.chart.template = function () {
		...
		var model = d2b.model.chart(update, ['zoomIn', 'zoomOut']);
		var $$ = model.values();
		var chart = model.base();
		...
		$$.legend.active(true).clickable(true).dblclickable(true).on('click', function () {
			console.log($$.legend.data());
		});

		// main chart update function
		function update() {
			$$.legend.data([{ label: 'Item 1 Long One' }, { label: 'Item 2' }, { label: 'Item 3 Here is a Longer One' }, { label: 'Item 4 Another' }, { label: 'Item 5' }, { label: 'Item 6 More' }, { label: 'Item 7' }]);
	...
	d2b.CHARTS.axisChart = function () {
		...
		//legend OBJ
		$$.legend = d2b.UTILS.LEGENDS.legend().active(true).color($$.color);
			...
			if (d2b.UTILS.AXISCHART.TYPES[type].tools) {
				...
				if (tools) {
					...
					if (tools.controlsData) {
						for (var control in tools.controlsData) {
		...
		$$.tooltip.update = function () {
			...
			$$.tooltip.x($$.axis.scale('x')).y($$.axis.scale('y')).start();
		};
		...
		$$.graphs.filterVisible = function (graph) {
			return graph.p.visible;
		};
		$$.graphs.init = function () {
			this.planes.forEach(function (plane) {
				//set graph data
				$$.selection.types[plane].type.graph = $$.selection.types[plane].type.selectAll('g.d2b-axis-chart-' + plane + '-graph').data(function (type) {
					return type.graphs.filter($$.graphs.filterVisible);
				}, function (graph) {
					return graph.p.key;
				});

				//enter new types and add reference to the persistent type data
				$$.selection.types[plane].type.graph.enter().append('g').attr('class', 'd2b-axis-chart-' + plane + '-graph').each(function (graph) {
					graph.p[plane] = this;
				}).style('opacity', 0);

				$$.selection.types[plane].type.graph.transition().duration($$.animationDuration).style('opacity', 1);

				$$.selection.types[plane].type.graph.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
			});
		};
		...
		$$.types.setProperties = function () {
			$$.currentChartData.types.forEach(function (type) {
				...
				var backgrounds = d3.selectAll(graphs.map(function (graph) {
					return graph.p.background;
				}));
				var foregrounds = d3.selectAll(graphs.map(function (graph) {
					return graph.p.foreground;
				}));
				d2b.UTILS.applyProperties.call(type.p.d2bType, type.properties);
				type.p.d2bType.animationDuration($$.animationDuration).x($$.axis.scale('x').reversed($$.rotate)).y($$.axis.scale('y').reversed($$.rotate)).width($$.innerWidth).height($$.innerHeight).color($$.color).tooltipSVG($$.tooltip).xFormat($$.xFormat).yFormat($$.yFormat).general(d3.select(type.p.general)).foreground(foregrounds).background(backgrounds).data(graphs);
			});
		};
		...
		$$.types.init = function () {
			...
			this.planes.forEach(function (plane) {
				//set type data
				$$.selection.types[plane].type = $$.selection.types[plane].selectAll('g.d2b-axis-type-' + plane).data($$.currentChartData.types.filter($$.types.filterVisible), function (d) {
					return d.type;
				});
				//enter new types and add reference to the persistent type data
				newPlanes[plane] = $$.selection.types[plane].type.enter().append('g').attr('class', function (d) {
					return 'd2b-axis-type-' + plane + ' d2b-' + d.type;
				}).each(function (type) {
					type.p[plane] = this;
				});
				//exit planes
				$$.selection.types[plane].type.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
			});

			//instantiate new d2b type object for entered types
			newPlanes.general.each(function (type) {
				type.p.d2bType = d2b.UTILS.AXISCHART.TYPES[type.type](type.type).axisChart(chart).controls($$.controlsData);
		...
		$$.axes.update = function () {
			...
			$$.selection.types.transition().duration($$.animationDuration).attr('transform', 'translate(' + axisBox.padding.left + ',' + axisBox.padding.top + ')');

			$$.selection.types.tracker.transition().duration($$.animationDuration).attr('width', axisBox.innerWidth).attr('height', axisBox.innerHeight);
		...
		$$.axes.setDomain = function () {
			...
			var xValues = [],
			    yValues = [];
			var xDomain, yDomain;
			var xScale = $$.axis.x().axis.scale(),
			    yScale = $$.axis.y().axis.scale();
		...
		$$.legend.updateData = function () {
			...
			if ($$.controlsData.hideLegend.enabled) {
				...
				$$.legendData.data.items = [].concat.apply([], $$.currentChartData.types.map(function (type) {
					return type.graphs.filter(function (graph) {
						return !graph.notInLegend;
					}).map(function (graph, i) {
						graph.open = !graph.p.visible;return graph;
					});
				}));
			}
		...
		$$.legend.mouseover = function (t, graph) {
			...
			$$.selection.types.foreground.type.graph.transition().duration($$.animationDuration / 2).style('opacity', 0.2);
			$$.selection.types.background.type.graph.transition().duration($$.animationDuration / 2).style('opacity', 0.2);
			d3.select(graph.p.background).transition().duration($$.animationDuration / 2).style('opacity', 1);
			d3.select(graph.p.foreground).transition().duration($$.animationDuration / 2).style('opacity', 1);
		};

		$$.legend.mouseout = function () {
			//reset dimming
			$$.selection.types.foreground.type.graph.transition().duration($$.animationDuration / 2).style('opacity', 1);
			$$.selection.types.background.type.graph.transition().duration($$.animationDuration / 2).style('opacity', 1);
		};
		...
		$$.transformTypesContainers = function () {
			if ($$.rotate) {
				$$.selection.types.background.transition().duration($$.animationDuration).attr('transform', 'translate(' + $$.innerWidth + ', 0),rotate(90)');
				$$.selection.types.foreground.transition().duration($$.animationDuration).attr('transform', 'translate(' + $$.innerWidth + ', 0),rotate(90)');
			} else {
				$$.selection.types.background.transition().duration($$.animationDuration).attr('transform', '');
				$$.selection.types.foreground.transition().duration($$.animationDuration).attr('transform', '');
			}
		};
		...
		chart.generate = function (callback) {
			...
			$$.controls.selection($$.selection.controls).on('change.d2b-change', function (d, i) {
				$$.controlsData[d.key].enabled = d.state;
				chart.update();
			});

			//init main chart container
			$$.selection.main = $$.selection.group.append('g').attr('class', 'd2b-axis-chart');

			$$.selection.axes = $$.selection.main.append('g').attr('class', 'd2b-axes');

			$$.selection.types = $$.selection.main.append('g').attr('class', 'd2b-axis-types').call($$.tooltip.tracker);
			$$.selection.types.tracker = $$.selection.types.append('rect').style('fill-opacity', 0);
			$$.selection.types.background = $$.selection.types.append('g').attr('class', 'd2b-axis-types-background');
			$$.selection.types.foreground = $$.selection.types.append('g').attr('class', 'd2b-axis-types-foreground');
			$$.selection.types.general = $$.selection.types.append('g').attr('class', 'd2b-axis-types-general');

			$$.selection.types.tooltip = $$.selection.types.append('g').attr('class', 'd2b-axis-types-tooltip-area').style('opacity', 0).call($$.tooltip.container);

			$$.legend.on('element-mouseover', $$.legend.mouseover).on('element-mouseout', $$.legend.mouseout).on('element-click', $$.legend.click).on('element-dblclick', $$.legend.dblClick);

			//auto update chart
			var temp = $$.animationDuration;
			return chart.animationDuration(0).update(callback).animationDuration(temp);
		};
		...
		chart.update = function (callback) {
			...
			$$.selection.group.transition().duration($$.animationDuration).attr('transform', 'translate(' + $$.forcedMargin.left + ',' + $$.forcedMargin.top + ')');
	...
	d2b.CHARTS.bubbleChart = function () {
		...
		$$.pack = d3.layout.pack().sort(function (a, b) {
			...
			return a.parent.parent && b.parent.parent ? d3.ascending(a.value, b.value) : null;
		});
		...
		$$.axis.x({ scale: $$.axis._x, orientation: 'top-outside' }).y({ scale: $$.axis._y, orientation: 'left-inside' });
		...
		$$.background = {
			...
			packs: function packs() {
				var textSize;
				var packLabel = this.selectAll('.d2b-pack-label').data($$.data.categories, function (d) {
				...
				newPackLabel = packLabel.enter().append('text').style('opacity', 0).attr('class', 'd2b-pack-label');

				newPackLabel.append('tspan').attr('class', 'd2b-pack-label-name');
				newPackLabel.append('tspan').attr('x', 0).attr('class', 'd2b-pack-label-value');

				$$.gridX.rangeBands([0, $$.outerWidth]).domain(d3.range(0, $$.grid.columns));
				$$.gridY.rangeBands([0, $$.outerHeight]).domain(d3.range(0, $$.grid.rows));

				packLabel.each(function (d, i) {
					d.center = {
				...
				// init pack labels
				newPackLabel.attr('transform', function (d) {
					var y = d.center.y - $$.gridY.rangeBand() / 3;
					return 'translate(' + d.center.x + ',' + y + ')';
				});

				packLabel.transition().duration($$.animationDuration).style('opacity', 1).attr('transform', function (d) {
					var y = d.center.y - $$.gridY.rangeBand() / 3;
					return 'translate(' + d.center.x + ',' + y + ')';
				});
				packLabel.select('tspan.d2b-pack-label-name').style('font-size', textSize + 'px').text(function (d) {
					return d.label;
				});
				packLabel.select('tspan.d2b-pack-label-value').style('font-size', textSize + 'px').attr('dy', textSize + 5).text(function (d) {
					return $$.xFormat(d.value);
				});
			...
			change: function change() {
				...
				var catLabels = $$.data.categories.map(function (d) {
					return d.label;
				});
				catLabels = d3.set(catLabels);

				$$.axis.width($$.outerWidth).height($$.outerHeight);
				...
				axisSvg.transition().duration($$.animationDuration).call($$.axis);
			}
		};
		...
		$$.zoom = {
			...
			toggle: function toggle() {
				if ($$.zoom.elem === this) $$.zoom.out();else $$.zoom.in.call(this);
			},
			// zoom in on a node
			in: function _in() {
				var self = this;
				...
				$$.selection.background.transition().duration($$.animationDuration).style('opacity', 0);

				$$.selection.bubbles.select('.d2b-bubbles-wrap').transition().duration($$.animationDuration).attr('transform', 'translate(0,0)');
				...
				}).classed('d2b-background-bubble', true).transition().duration($$.animationDuration).call($$.bubbles.transition, true);

				// update sub-bubbles again after zoom attrs have been changed
				elem.classed('d2b-background-bubble', false).each($$.bubbles.updateSubPack);
			},
			// zoom out
			out: function out() {
				if (!$$.zoom.elem) return;
				...
				elem.selectAll('.d2b-sub-bubble').transition().duration($$.animationDuration).style('opacity', 0).call($$.bubbles.transition).remove();

				$$.selection.bubbles.selectAll('.d2b-bubble').classed('d2b-background-bubble', false);
			}
		};
		...
		$$.bubbles = {
			...
			getSubColor: function getSubColor(d, parentColor) {
				if (d.colorKey !== null && d.colorKey !== undefined) return $$.color(d.colorKey);else if (d.children && d.children.length) return d3.rgb(parentColor).brighter(d.depth / 2);else return 'white';
			},
			// update the subpack for the zoomed in node
			updateSubPack: function updateSubPack(d) {
				var subBubble,
				    newSubBubble,
				    newCircle,
				...
				subBubble = d3.select(bubble.node()).selectAll('.d2b-sub-bubble').data(pack, function (d) {
					return d.key || d.label;
				});

				newSubBubble = subBubble.enter().append('g').attr('class', 'd2b-sub-bubble').style('opacity', 0);

				newCircle = newSubBubble.append('circle').call(d2b.UTILS.bindTooltip, $$.tooltipContent, function (d) {
					return d;
				}, function (d) {
				...
				newCircle.style('fill', function (d) {
					return d.attrs.color;
				}).style('stroke', function (d) {
					return d3.rgb(d.attrs.color).darker(1);
				});
				...
				subBubble.transition().duration($$.animationDuration).style('opacity', 1).call($$.bubbles.transition, true);
			},
			// transition a set of bubbles (radius, translation, fill, stroke)
			transition: function transition(bubble, zoom) {
			...
			packs: function packs() {
				...
				bubblesWrap.transition().duration($$.animationDuration).attr('transform', 'translate(0,0)');
			...
			change: function change() {
				...
				var getX = function getX(d) {
					return Math.min(Math.max(x(d), 0), box.innerWidth);
				};
				var bubblesWrap = this.select('.d2b-bubbles-wrap');
				var bubble = this.selectAll('.d2b-bubble');

				bubblesWrap.transition().duration($$.animationDuration).attr('transform', 'translate(' + box.padding.left + ', ' + box.padding.top + ')');
		...
		chart.zoom = function (elem) {
			var zoomed = false;
			$$.selection.bubbles.selectAll('.d2b-bubble').each(function (d) {
				if (!zoomed && (elem === d || elem === this || elem === d.label || elem === d.key)) {
					zoomed = true;
					$$.zoom.in.call(this);
				}
			});
		...
		chart.generate = function (callback) {
			...
			//init main chart container
			$$.selection.main = $$.selection.group.append('g').attr('class', 'd2b-bubble-chart');

			$$.selection.bubbles = $$.selection.main.append('g').attr('class', 'd2b-bubbles');

			$$.selection.bubbles.append('g').attr('class', 'd2b-bubbles-wrap');

			$$.selection.background = $$.selection.main.append('g').attr('class', 'd2b-bubble-background');
		...
		chart.update = function (callback) {
			...
			$$.selection.background.transition().duration($$.animationDuration).style('opacity', 1);
			background = $$.selection.background.selectAll('.d2b-bubble-background-wrap').data([$$.view], function (d) {
				return d;
			});

			background.enter().append('g').attr('class', 'd2b-bubble-background-wrap');
			background.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();

			// enter update exit bubbles
			bubbleData = $$.data.categories.map(function (d) {
				return d.children;
			});
			bubbleData = [].concat.apply([], bubbleData).sort(function (a, b) {
				return d3.descending(a.r, b.r);
			});

			bubble = $$.selection.bubbles.select('.d2b-bubbles-wrap').selectAll('.d2b-bubble').data(bubbleData, function (d) {
				return d.key || d.label;
			});

			var newBubble = bubble.enter().append('g').attr('class', 'd2b-bubble');

			newBubble.append('circle').call(d2b.UTILS.bindTooltip, $$.tooltipContent, function (d) {
				return d;
			}, function (d) {
				return d.attrs.color;
			});

			bubble.classed('d2b-zoomable', false).on('click.d2b-bubble-zoom', null).each(function () {
				this.parentNode.appendChild(this);
			}).filter(function (d) {
				return d.children && d.children.length;
			}).classed('d2b-zoomable', true).on('click.d2b-bubble-zoom', $$.zoom.toggle);

			// remove exited bubbles
			bubble.exit().transition().duration($$.animationDuration).remove().select('circle').attr('r', 0);
			...
			// init new bubble
			newBubble.call($$.bubbles.transition).select('circle').attr('r', 0);

			// transition bubbles after attributes have been set
			bubble.transition().duration($$.animationDuration).call($$.bubbles.transition);
	...
	d2b.CHARTS.factChart = function () {
		...
		$$.textScale.custom = function (value) {
			...
			if (translation < range[0]) return range[0];else if (translation > range[1]) return range[1];
			return translation;
		};

		$$.updateFact = function (fact, textSize) {
			fact.select('text.d2b-fact-label').style('font-size', textSize + 'px').text(function (d) {
				return d.label;
			}).each(function (d) {
				d.labelWidth = this.getComputedTextLength();
			}).transition().duration($$.animationDuration).attr('y', -0.5 * textSize);

			var barHeight = 3;
			fact.select('rect').transition().duration($$.animationDuration).attr('width', function (d) {
				return d.labelWidth;
			}).attr('height', barHeight).attr('x', function (d) {
				return -d.labelWidth / 2;
			}).attr('y', -barHeight / 2).style('fill', '#ccc');

			fact.select('text.d2b-fact-value').style('font-size', 2 * textSize + 'px').text(function (d) {
				return $$.xFormat(d.value);
			}).transition().duration($$.animationDuration).attr('y', 2 * textSize);
		};

		$$.showMainFacts = function () {
			$$.selection.subFacts.style('pointer-events', 'none').selectAll('*').transition().duration($$.animationDuration).style('opacity', 0).remove();
			...
			var newFact = $$.selection.fact.enter().append('g').attr('class', 'd2b-fact').call($$.events.addElementDispatcher, 'main', 'd2b-fact');
			...
			var factTransition = $$.selection.fact.style('pointer-events', 'all').transition().duration($$.animationDuration).style('opacity', 1).attr('transform', function (d, i) {
			...
			$$.selection.fact.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
			...
			$$.selection.fact.each(function (d) {
				...
				if (!d.subFacts || d.subFacts.length < 1) {
					elem.style('cursor', 'auto').on('mouseover.d2b-mouseover', null).on('mouseout.d2b-mouseout', null).on('click.d2b-click', null);
					return;
				}

				elem.style('cursor', 'pointer').on('mouseover.d2b-mouseover', function () {
					elem.select('rect').transition().duration($$.animationDuration / 2).style('fill', '#333');
				}).on('mouseout.d2b-mouseout', function () {
					elem.select('rect').transition().duration($$.animationDuration / 2).style('fill', '#ccc');
				}).on('click.d2b-click', function () {
					var elem = d3.select(this);
					if ($$.selectedFact) {
						$$.selection.fact.style('pointer-events', 'all').transition().duration($$.animationDuration).style('opacity', 1);
						$$.selectedFact = null;
					} else {
						$$.selection.fact.style('pointer-events', 'none').transition().duration($$.animationDuration).style('opacity', 0);
						elem.style('pointer-events', 'all').transition().duration($$.animationDuration).style('opacity', 1);
						$$.selectedFact = elem;
						$$.selectedFact.data = d;
					}
					chart.update();
		...
		$$.showSubFacts = function () {
			...
			$$.selectedFact.transition().duration($$.animationDuration).attr('transform', 'translate(' + $$.innerWidth / 2 + ',' + textSize + ')');
			...
			$$.selection.subFacts.style('pointer-events', 'all').transition().duration($$.animationDuration).style('opacity', 1).attr('transform', 'translate(0,' + topOffset + ')');
			...
			var newSubFact = $$.selection.subFact.enter().append('g').attr('class', 'd2b-sub-fact').style('opacity', 0).attr('transform', function (d, i) {
				return 'translate(' + $$.x(i % grid.columns) + ',' + $$.y(Math.floor(i / grid.columns)) + ')';
			});

			newSubFact.append('g').attr('class', 'd2b-sub-fact-chart').each(function (d) {
				this.pieChart = new d2b.CHARTS.pieChart();
				this.pieChart.select(this).color($$.color).donutRatio(0.6).controls({ hideLegend: { enabled: true } });
			});

			newSubFact.append('text').attr('class', 'd2b-sub-fact-label');

			$$.selection.subFact.transition().duration($$.animationDuration).attr('transform', function (d, i) {
				return 'translate(' + $$.x(i % grid.columns) + ',' + $$.y(Math.floor(i / grid.columns)) + ')';
			}).delay($$.animationDuration / 1.5).style('opacity', 0.9);

			$$.selection.subFact.select('g.d2b-sub-fact-chart').each(function (d) {
				$$.events.translateEvents(this.pieChart);
				this.pieChart.animationDuration($$.animationDuration).width($$.x.rangeBand()).height($$.y.rangeBand()).data({ data: { values: d.values } }).update();
			});

			$$.selection.subFact.select('text.d2b-sub-fact-label').text(function (d) {
				return d.label;
			}).transition().duration($$.animationDuration).attr('x', $$.x.rangeBand() / 2).attr('y', $$.y.rangeBand() / 2);
		};
		...
		chart.generate = function (callback) {
			...
			//create svg
			$$.selection.svg = $$.selection.append('svg').attr('class', 'd2b-svg d2b-container');

			//create group container
			$$.forcedMargin = d2b.CONSTANTS.DEFAULTFORCEDMARGIN();
			$$.selection.group = $$.selection.svg.append('g').attr('transform', 'translate(' + $$.forcedMargin.left + ',' + $$.forcedMargin.top + ')');

			//init main chart container
			$$.selection.main = $$.selection.group.append('g').attr('class', 'd2b-fact-chart');

			$$.selection.subFacts = $$.selection.main.append('g').attr('class', 'd2b-sub-facts');
	...
	d2b.CHARTS.funnelChart = function () {
		...
		$$.funnel = function () {
			...
			$$.coneHeight.range([0, $$.innerHeight * $$.heightCoeff]).domain([0, d3.sum($$.currentChartData.values, function (d) {
				return d.value;
			})]);
			...
			$$.currentChartData.values.forEach(function (d) {
				...
				d.center = $$.innerWidth / 2 - topWidth / 2 + "," + 0 + " " + ($$.innerWidth / 2 - bottomWidth / 2) + "," + height + " " + ($$.innerWidth / 2 + bottomWidth / 2) + "," + height + " " + ($$.innerWidth / 2 + topWidth / 2) + "," + 0 + " ";
		...
		$$.showText = function (d) {
			// console.log(this)
			this.select('.d2b-cone-note-group').style('opacity', 0).attr('transform', function (d) {
				return 'translate(' + $$.innerWidth / 2 + ',' + d.height + ')';
			});
			this.select('.d2b-cone-text-group').style('opacity', 1).attr('transform', function (d) {
				return 'translate(' + $$.innerWidth / 2 + ',' + (d.height / 2 + $$.coneCurve - 15) + ')';
			});
		};

		$$.showNote = function (d) {
			this.select('.d2b-cone-note-group').style('opacity', 1).attr('transform', function (d) {
				return 'translate(' + $$.innerWidth / 2 + ',' + ($$.coneCurve * 1.5 + (d.height - d.noteHeight) / 2) + ')';
			});
			this.select('.d2b-cone-text-group').style('opacity', 0).attr('transform', function (d) {
		...
		$$.coneMouseover = function (d) {
			...
			$$.selection.funnel.cone.each(function () {
				var transition = d3.select(this).transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short * 2);
				if (this == _self) {
					...
					transition.attr('transform', function (d) {
						return 'translate(0,' + (d.y + mouseoverPadding) + ')';
					});
				} else {
					transition.attr('transform', function (d) {
						return 'translate(0,' + (d.y - mouseoverPadding) + ')';
		...
		$$.coneMouseout = function (d) {
			$$.selection.funnel.cone.transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short * 2).call($$.showText).attr('transform', function (d) {
				return 'translate(0,' + d.y + ')';
			});
		};

		$$.enterCones = function () {
			var newCone = $$.selection.funnel.cone.enter().append('g').attr('class', 'd2b-cone').style('opacity', 0).call($$.events.addElementDispatcher, 'main', 'd2b-cone').on('mouseout.d2b-mouseout', $$.coneMouseout).on('mouseover.d2b-mouseover', $$.coneMouseover);

			newCone.append('path').attr('class', 'd2b-cone-bottom');
			newCone.append('polygon').attr('class', 'd2b-cone-center');
			newCone.append('path').attr('class', 'd2b-cone-top');

			var newConeNoteGroup = newCone.append('g').attr('class', 'd2b-cone-note-group');

			var newConeNote = newConeNoteGroup.append('text').attr('class', 'd2b-cone-note');

			var newConeTextGroup = newCone.append('g').attr('class', 'd2b-cone-text-group');

			var newConeText = newConeTextGroup.append('text').attr('class', 'd2b-cone-text');

			newConeText.append('tspan').attr('class', 'd2b-cone-label');

			newConeText.append('tspan').attr('class', 'd2b-cone-value').attr('x', 0).attr('y', 20);
		...
		$$.updateCones = function () {
			...
			$$.selection.funnel.transition().duration($$.animationDuration).attr('transform', function () {
				return 'translate(0,' + $$.innerHeight * (1 - $$.heightCoeff) / 2 + ')';
			});

			var coneTransition = $$.selection.funnel.cone.transition().duration($$.animationDuration).attr('transform', function (d) {
				return 'translate(0,' + d.y + ')';
			}).style('opacity', 1).call($$.showText);

			// coneTransition.select('.d2b-cone-note-group')
			// 		.attr('transform',function(d){return 'translate('+$$.innerWidth/2+','+($$.coneCurve*1.5)+')';})
			$$.selection.funnel.cone.note.attr('dy', 0).attr('y', 0).text(function (d) {
				return d.note;
			}).each(function (d) {
				d3.select(this).call(d2b.UTILS.textWrap, d.width - 10);
			...
			coneTransition.select('.d2b-cone-label').text(function (d) {
				return d.label;
			});
			coneTransition.select('.d2b-cone-value').text(function (d) {
				return $$.xFormat(d.value);
			});

			coneTransition.select('.d2b-cone-bottom').attr('d', function (d) {
				return d.bottom;
			}).style('fill', function (d) {
				return $$.color(d.label);
			});

			coneTransition.select('.d2b-cone-center').attr('points', function (d) {
				return d.center;
			}).style('fill', function (d) {
				return $$.color(d.label);
			});

			coneTransition.select('.d2b-cone-top').attr('d', function (d) {
				return d.top;
			}).style('fill', function (d) {
				return d3.rgb($$.color(d.label)).darker(1.2);
			});
		};

		$$.exitCones = function () {
			$$.selection.funnel.cone.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
		};
		...
		chart.generate = function (callback) {
			...
			//init main chart container
			$$.selection.main = $$.selection.group.append('g').attr('class', 'd2b-funnel-chart');

			$$.selection.funnel = $$.selection.main.append('g').attr('class', 'd2b-funnel');
		...
		chart.update = function (callback) {
			...
			$$.selection.funnel.cone = $$.selection.funnel.selectAll('g.d2b-cone').data($$.funnel($$.currentChartData.values)._reverse(), function (d, i) {
				if (d.key == 'unique') return Math.floor((1 + Math.random()) * 0x10000);else if (d.key && d.key != 'auto') return d.key;else return d.label;
			});
	...
	d2b.CHARTS.gaugeChart = function () {
		...
		$$.tooltip = function (d) {
			if (d.value != null) return "<b>" + d.label + ":</b> " + $$.xFormat(d.value) + " (" + d.percent * 100 + "%)";else return "<b>" + d.label + ":</b> " + d.percent * 100 + "%";
		};
		...
		$$.getPercent = function () {
			...
			if ($$.currentChartData.percent != null) return $$.currentChartData.percent;else if ($$.currentChartData.value != null && $$.currentChartData.total != null) return $$.currentChartData.value / $$.currentChartData.total;else return 0;
		};
		...
		$$.arcMouseover = function (d) {
			...
			arc.call($$.setNewArc, $$.radius.inner, $$.radius.outer * 1.03).transition().duration($$.animationDuration / 4).call(d2b.UTILS.TWEENS.arcTween, $$.arc);
		};

		$$.arcMouseout = function (d) {
			var arc = d3.select(this).select('path');
			arc.call($$.setNewArc, $$.radius.inner, $$.radius.outer).transition().duration($$.animationDuration / 4).call(d2b.UTILS.TWEENS.arcTween, $$.arc);
		};
		...
		chart.generate = function (callback) {
			...
			//create svg
			$$.selection.svg = $$.selection.append('svg').attr('class', 'd2b-template-chart d2b-svg d2b-container');
			...
			//init main chart container
			$$.selection.main = $$.selection.group.append('g').attr('class', 'd2b-gauge-chart');

			$$.selection.arcs = $$.selection.main.append('g');

			$$.selection.arcHeader = $$.selection.main.append('text').attr('class', 'd2b-gauge-arc-header');

			$$.selection.arcLabels = $$.selection.main.append('g').attr('class', 'd2b-gauge-arc-labels');

			$$.selection.arcLabels.start = $$.selection.arcLabels.append('text').attr('y', 20).text('0%');
			$$.selection.arcLabels.end = $$.selection.arcLabels.append('text').attr('y', 20).text('100%');
			$$.selection.arcLabels.percent = $$.selection.arcLabels.append('text').attr('class', 'd2b-gauge-arc-percent').text('0%');
		...
		chart.update = function (callback) {
			...
			var newArc = $$.selection.arcs.arc.enter().append('g');
			newArc.append('path')
			// .attr('d', $$.arc)
			.style('fill', function (d) {
				return d.color;
			...
			newArc.filter(function (d) {
				return d.filled;
			}).on('mouseover.d2b-mouseover', $$.arcMouseover).on('mouseout.d2b-mouseout', $$.arcMouseout).call($$.events.addElementDispatcher, 'main', 'd2b-arc');

			$$.selection.arcs.arc.filter(function (d) {
				return d.filled;
			}).call(d2b.UTILS.bindTooltip, $$.tooltip, function (d) {
				return d;
			});

			$$.selection.arcs.arc.path = $$.selection.arcs.arc.select('path').call($$.setNewArc, $$.radius.inner, $$.radius.outer).transition().duration($$.animationDuration).call(d2b.UTILS.TWEENS.arcTween, $$.arc).style('fill', function (d) {
				return d.color;
			}).attr('class', 'd2b-arc');

			$$.selection.arcLabels.percent.transition().duration($$.animationDuration).tween("text", function (d) {
				var _self = this;
				if (!_self._current) _self._current = 0;
			...
			$$.selection.arcLabels.start.transition().duration($$.animationDuration).attr('x', -($$.radius.outer - ($$.radius.outer - $$.radius.inner) / 2));
			$$.selection.arcLabels.end.transition().duration($$.animationDuration).attr('x', $$.radius.outer - ($$.radius.outer - $$.radius.inner) / 2);

			$$.selection.arcHeader.text($$.currentChartData.label).transition().duration($$.animationDuration).attr('y', -$$.radius.outer - 10);
	...
	d2b.CHARTS.iframeChart = function () {
		...
		chart.generate = function (callback) {
			...
			$$.selection.div = $$.selection.append('div').attr('class', 'd2b-iframe-chart d2b-container');

			$$.selection.div.iframe = $$.selection.div.append('iframe').attr('class', 'd2b-iframe').attr('src', $$.currentChartData.url);
		...
		chart.update = function (callback) {
			...
			$$.selection.div.iframe.transition().duration($$.animationDuration).attr('width', $$.width).attr('height', $$.height);
	...
	d2b.CHARTS.mapChart = function () {
		...
		$$.topoPaths = [{ key: 'countries', file: 'countries.topo.json' }, { key: 'states', file: 'states.topo.json' }
		...
		$$.drawCountries = function (path) {
			...
			$$.selection.countries.country = $$.selection.countries.selectAll('g.d2b-country').data($$.filteredTopoData.countries.features, function (d) {
				return d.properties.GU_A3;
			});

			$$.selection.countries.country.enter().append("g").attr('class', 'd2b-country').style("opacity", 0).append("path");

			$$.selection.countries.country.path = $$.selection.countries.country.select('path');

			$$.selection.countries.country.transition().duration($$.animationDuration).style("opacity", 1).select('path').attr("d", path).style("fill", "black");

			$$.selection.countries.country.exit().transition().duration($$.animationDuration).style("opacity", 0).remove();
		};
		...
		$$.pathFit = function (feature) {
			...
			var projection = d3.geo[$$.projection]().scale(scale).translate(offset);

			if ($$.projection != "albersUsa") projection.center(center);
			...
			var offset = [$$.innerWidth - (bounds[0][0] + bounds[1][0]) / 2, $$.innerHeight - (bounds[0][1] + bounds[1][1]) / 2];
		...
		$$.setTopoData = function (callback) {
			...
			d2b.UTILS.load($$.topoPaths, function (data) {
				...
				$$.filteredTopoData.states.objects.states_simplified.geometries = $$.topoData.states.objects.states_simplified.geometries.filter(function (geo) {
					return Object.keys($$.filterSet.states || {}).map(function (key) {
						return $$.filterSet.states[key].split(',').indexOf(geo.properties[key]) > -1;
					}).concat([true]).every(function (test) {
						return test == true;
					});
				});
				...
				$$.filteredTopoData.countries.objects.countries_simplified.geometries = $$.topoData.countries.objects.countries_simplified.geometries.filter(function (geo) {
		...
		chart.generate = function (callback) {
			...
			$$.selection.countries = $$.selection.main.append('g').attr('class', 'd2b-countries');
	...
	d2b.CHARTS.multiChart = function () {
		...
		$$.width = d2b.CONSTANTS.DEFAULTWIDTH(), $$.height = d2b.CONSTANTS.DEFAULTHEIGHT();
		...
		var buttonClick = function buttonClick(d, i) {
		...
		var updateButtons = function updateButtons() {
			...
			$$.selection.buttonsWrapper.buttons.button.enter().append('li').on('click.d2b-click', buttonClick);

			$$.selection.buttonsWrapper.buttons.button.text(function (d) {
				return d.label;
			}).classed('d2b-selected', function (d) {
				return d == $$.current.chart;
			});

			$$.selection.buttonsWrapper.buttons.button.exit().remove();
		};

		var setChartProperties = function setChartProperties(options) {
			if ($$.current.chart.properties) {
				for (var key in $$.current.chart.properties) {
					if (options && options.skip) {
						if (options.skip.indexOf(key) > -1) continue;
					}
					if (key == 'data') continue;
					if ($$.current.chart.properties[key].args) $$.d2bChart[key].apply(this, $$.current.chart.properties[key].args);else $$.d2bChart[key]($$.current.chart.properties[key]);
				}
			}
		...
		var updateChart = function updateChart() {
			if (!$$.selection.chartWrapper.chart) {
				$$.selection.chartWrapper.chart = $$.selection.chartWrapper.append('div').attr('class', 'd2b-multi-chart-chart').style('opacity', 1);
				// d2b.UTILS.chartAdapter($$.current.chart.type, $$.current.chart);
				$$.d2bChart = $$.current.chart.chart;
				$$.d2bChart.data($$.current.chart.properties.data).selection($$.selection.chartWrapper.chart);

				setChartProperties();
			} else if ($$.current.chart != $$.previous.chart) {
				if ($$.current.chart.type == $$.previous.chart.type) {
					$$.d2bChart.data($$.current.chart.properties.data);

					setChartProperties({ 'skip': ['controls'] });
				} else {
					$$.selection.chartWrapper.chart.transition().duration($$.animationDuration).style('opacity', 0).remove();

					$$.selection.chartWrapper.chart = $$.selection.chartWrapper.append('div').attr('class', 'd2b-multi-chart-chart').style('opacity', 0);

					// d2b.UTILS.chartAdapter($$.current.chart.type, $$.current.chart);
					$$.d2bChart = $$.current.chart.chart;
					$$.d2bChart.selection($$.selection.chartWrapper.chart).data(JSON.parse(JSON.stringify($$.current.chart.data))); //clone data for update

					$$.selection.chartWrapper.chart.transition().duration($$.animationDuration).style('opacity', 1);
			...
			$$.d2bChart
			// .events($$.events)
			.width($$.innerWidth).height($$.innerHeight).update();
		...
		chart.data = function (chartData, reset) {
			...
			if (useDefault) $$.current.chart = chartData.data.charts[0];
		...
		chart.generate = function (callback) {
			...
			$$.selection.buttonsWrapper = $$.selection.append('div').attr('class', 'd2b-multi-chart-buttons-wrapper');

			$$.selection.buttonsWrapper.buttons = $$.selection.buttonsWrapper.append('ul').attr('class', 'd2b-buttons');

			// $$.selection.style('position','relative');
			$$.selection.chartWrapper = $$.selection.append('div').attr('class', 'd2b-multi-chart d2b-container');
		...
		chart.update = function (callback) {
			...
			$$.selection.chartWrapper.style('width', $$.innerWidth + 'px').style('height', $$.innerHeight + 'px');
	...
	d2b.CHARTS.pieChart = function () {
		...
		$$.arc = d3.svg.arc().startAngle(function (d) {
			return Math.max(0, Math.min(2 * Math.PI, d.start));
		}).endAngle(function (d) {
			return Math.max(0, Math.min(2 * Math.PI, d.end));
		}).outerRadius(function (d) {
			return Math.max(0, d.outer);
		}).innerRadius(function (d) {
			return Math.max(0, d.inner);
		});
		...
		$$.arcEnter = function () {
			...
			$$.selection.pie.arc = $$.selection.pie.selectAll('g').data($$.pie, function (d) {
				return d.data.key;
			});

			//arc enter
			var newArc = $$.selection.pie.arc.enter().append('g').attr('class', 'd2b-arc').style('opacity', 0).call($$.events.addElementDispatcher, 'main', 'd2b-arc');

			//create arc path
			newArc.append('path').each(function (d) {
				//init old Arc
				this.oldArc = { start: d.startAngle, end: d.startAngle, inner: $$.r * $$.donutRatio, outer: $$.r };
			});

			//create arc text
			newArc.append('text').attr('y', 6).each(function (d) {
		...
		$$.arcUpdate = function () {
			...
			$$.selection.pie.arc.call(d2b.UTILS.bindTooltip, $$.tooltipContent, function (d) {
				return d.data;
			}, function (d) {
				return d2b.UTILS.getColor($$.color, 'label')(d.data);
			}).transition().duration($$.animationDuration / 1.5).style('opacity', 1);

			//update arc path
			$$.selection.pie.arc.path = $$.selection.pie.arc.select('path').each(function (d) {
			...
			$$.selection.pie.arc.path.transition = $$.selection.pie.arc.path.transition().duration($$.animationDuration / 1.5).style('opacity', 1).style('fill', function (d) {
				return d2b.UTILS.getColor($$.color, 'label')(d.data);
			}).call(d2b.UTILS.TWEENS.arcTween, $$.arc);

			// arc text
			$$.selection.pie.arc.text = $$.selection.pie.arc.select('text').transition().duration($$.animationDuration / 1.5).tween("text", function (d) {
				//tween percent value
				var _self = this;
				if (!_self._current) _self._current = 0;
		...
		$$.arcExit = function () {
			$$.selection.pie.arc.exit().transition().duration($$.animationDuration).style('opacity', 0).remove().select('path').each(function (d) {
		...
		chart.generate = function (callback) {
			...
			//init legend properties
			$$.legend.color($$.color).selection($$.selection.legend).on('element-mouseover.d2b-mouseover', function (t, d) {
				...
				$$.currentChartData.values.forEach(function (d) {
					if (allHidden == true && !$$.persistentData.hiddenArcs[d.key]) allHidden = false;
				});
				//if all types/graphs are hidden, show them all
				if (allHidden) {
			...
			$$.selection.pie = $$.selection.group.append('g').attr('class', 'd2b-pie-chart');
		...
		chart.update = function (callback) {
			...
			$$.selection.pie.transition().duration($$.animationDuration).attr('transform', 'translate(' + ($$.forcedMargin.left + $$.innerWidth / 2) + ',' + ($$.forcedMargin.top + $$.innerHeight / 2) + ')');
	...
	d2b.CHARTS.sankeyChart = function () {
		...
		chart.generate = function (callback) {
			...
			//create svg
			$$.selection.svg = $$.selection.append('svg').attr('class', 'd2b-sankey-chart d2b-svg d2b-container');

			//create group container
			$$.selection.group = $$.selection.svg.append('g');

			$$.selection.group.sankey = $$.selection.group.append('g').attr('class', 'd2b-sankey');
			$$.selection.group.sankey.links = $$.selection.group.sankey.append('g').attr('class', 'd2b-sankey-links');
			$$.selection.group.sankey.nodes = $$.selection.group.sankey.append('g').attr('class', 'd2b-sankey-nodes');

			$$.selection.group.labels = $$.selection.group.append('g').attr('class', 'd2b-sankey-labels');

			$$.selection.group.labels.source = $$.selection.group.labels.append('g').attr('class', 'd2b-sankey-label-source');

			$$.selection.group.labels.source.text = $$.selection.group.labels.source.append('text').attr('y', 23);

			$$.selection.group.labels.destination = $$.selection.group.labels.append('g').attr('class', 'd2b-sankey-label-destination');

			$$.selection.group.labels.destination.text = $$.selection.group.labels.destination.append('text').attr('y', 23);

			$$.selection.group.columnHeaders = $$.selection.group.append('g').attr('class', 'd2b-sankey-column-headers');

			//create controls container
			$$.selection.controls = $$.selection.group.append('g').attr('class', 'd2b-controls');
			...
			$$.selection.legend = $$.selection.group.append('g').attr('class', 'd2b-legend');
		...
		chart.update = function (callback) {
			...
			if ($$.controlsData.hideLegend.enabled) {
				...
				$$.legendData = {
					data: {
						items: d3.set($$.currentChartData.nodes.map(function (d) {
			...
			var labelTransitions = {
				source: $$.selection.group.labels.source.transition().duration($$.animationDuration),
				destination: $$.selection.group.labels.destination.transition().duration($$.animationDuration)
			};

			if ($$.currentChartData.labels) {
				$$.selection.group.labels.transition().duration($$.animationDuration).attr('transform', 'translate(' + $$.forcedMargin.left + ',' + $$.forcedMargin.top + ')');
				if ($$.currentChartData.labels.source) {
					labelTransitions.source.style('opacity', 1);
					$$.selection.group.labels.source.text.text($$.currentChartData.labels.source);
				} else {
					labelTransitions.source.style('opacity', 0);
				}
				if ($$.currentChartData.labels.destination) {
					labelTransitions.destination.style('opacity', 1);
					$$.selection.group.labels.destination.text.text($$.currentChartData.labels.destination);
				} else {
					labelTransitions.destination.style('opacity', 0);
				}

				labelTransitions.source.attr('transform', 'translate(' + 0 + ',' + 0 + ')');
				labelTransitions.destination.attr('transform', 'translate(' + $$.innerWidth + ',' + 0 + ')');
			...
			if ($$.currentChartData.columnHeaders && $$.currentChartData.columnHeaders.length > 0) {
				columnHeaderScale = d3.scale.linear().domain([0, $$.currentChartData.columnHeaders.length - 1]).range([0, $$.innerWidth - $$.nodeWidth]);

				$$.selection.group.columnHeaders.transition().duration($$.animationDuration).attr('transform', 'translate(' + $$.forcedMargin.left + ',' + $$.forcedMargin.top + ')');

				columnHeader = $$.selection.group.columnHeaders.selectAll('g.d2b-sankey-column-header').data($$.currentChartData.columnHeaders);
				columnHeader.enter().append('g').style('opacity', 0).attr('transform', function (d, i) {
					return 'translate(' + (columnHeaderScale(i) + $$.nodeWidth / 2) + ',' + 0 + ')';
				}).attr('class', 'd2b-sankey-column-header').append('text').attr('y', 16).attr('x', function (d, i) {
					if (i == 0) return -$$.nodeWidth / 2;else if (i == $$.currentChartData.columnHeaders.length - 1) return $$.nodeWidth / 2;
				});

				columnHeader.select('text').text(function (d) {
					return d;
				});
				columnHeader.transition().duration($$.animationDuration).style('opacity', 1).attr('transform', function (d, i) {
					return 'translate(' + (columnHeaderScale(i) + $$.nodeWidth / 2) + ',' + 0 + ')';
				});

				columnHeader.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
			...
			$$.sankey.size([$$.innerWidth, $$.innerHeight]).nodeWidth($$.nodeWidth).nodePadding($$.nodePadding).nodes($$.currentChartData.nodes).links($$.currentChartData.links).layout($$.layout);

			var node = $$.selection.group.sankey.nodes.selectAll('g.d2b-sankey-node').data($$.currentChartData.nodes.filter(function (d) {
				return d.value;
			}), function (d, i) {
				if (d.key == 'unique') return Math.floor((1 + Math.random()) * 0x10000);else if (d.key && d.key != 'auto') return d.key;else return i;
			});
			var newNode = node.enter().append('g').attr('class', 'd2b-sankey-node').attr('transform', function (d) {
				return 'translate(' + d.x + ',' + d.y + ')';
			}).style('opacity', 0).call($$.events.addElementDispatcher, 'main', 'd2b-sankey-node');
			newNode.append('rect');
			newNode.append('text');

			var nodeText = node.select('text').text(function (d) {
				return d.shortName;
			});

			node.call(d2b.UTILS.bindTooltip, $$.nodeTooltip, function (d) {
				return d;
			}).transition().duration($$.animationDuration).attr('transform', function (d) {
				return 'translate(' + d.x + ',' + d.y + ')';
			}).style('opacity', 1);

			node.select('rect').transition().duration($$.animationDuration).attr('width', $$.sankey.nodeWidth()).attr('height', function (d) {
				return Math.max(0, d.dy);
			});
			nodeText.transition().duration($$.animationDuration).style('text-anchor', function (d) {
				return d.x < $$.innerWidth / 2 ? 'start' : 'end';
			}).attr('x', function (d) {
				return d.x < $$.innerWidth / 2 ? $$.sankey.nodeWidth() + 5 : -5;
			}).attr('y', function (d) {
				return d.dy / 2 + 5;
			});

			var link = $$.selection.group.sankey.links.selectAll('g.d2b-sankey-link').data($$.currentChartData.links.filter(function (d) {
				return d.value;
			}), function (d, i) {
				if (d.key == 'unique') return Math.floor((1 + Math.random()) * 0x10000);else if (d.key && d.key != 'auto') return d.key;else {
					return i;
				}
			});
			var newLink = link.enter().append('g').attr('class', 'd2b-sankey-link').style('opacity', 0).call($$.events.addElementDispatcher, 'main', 'd2b-sankey-node');
			...
			node.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();

			link.call(d2b.UTILS.bindTooltip, $$.linkTooltip, function (d) {
				return d;
			}).transition().duration($$.animationDuration).style('opacity', 1);

			link.select('path').style('stroke', function (d) {
				return d.colorBy ? $$.color(d[d.colorBy].colorKey ? d[d.colorBy].colorKey : d[d.colorBy].name) : '#777';
			}).transition().style('stroke-width', function (d) {
				return Math.max(d.dy, $$.minLinkWidth);
			}).duration($$.animationDuration).attr('d', $$.sankey.link());

			$$.selection.svg.attr('width', $$.width).attr('height', $$.height);

			$$.selection.group.sankey.transition().duration($$.animationDuration).attr('transform', 'translate(' + $$.forcedMargin.left + ',' + $$.forcedMargin.top + ')');
	...
	d2b.CHARTS.sunburstChart = function () {
		...
		$$.arcFill = function (d) {
			...
			if (d.colorKey) return $$.color(d.colorKey);
		...
		$$.setSunburstTooltip = function (d, showPercent) {
			...
			if (showPercent) tspanValue += ' / ' + d3.format(".2%")(d.value / $$.currentRoot.value);

			$$.selection.tooltip.text.selectAll('*').remove();

			$$.selection.tooltip.text.append('tspan').text(tspanName);
			$$.selection.tooltip.text.append('tspan').attr('y', 30).attr('x', 0).text(tspanValue);
		};
		...
		$$.getZoomParentDomain = function (d) {
			...
			var domain = [1, 0];
			do {
				if (domain[0] > cur.y) domain[0] = cur.y;
				if (domain[1] < cur.y + cur.dy) domain[1] = cur.y + cur.dy;
				cur = cur.parent;
			} while (cur);
			return domain;
		};
		...
		$$.getZoomChildDomain = function (d, domain) {
			if (!domain) {
				...
				if (domain[0] > d.y) domain[0] = d.y;
				if (domain[1] < d.y + d.dy) domain[1] = d.y + d.dy;
			}
		...
		$$.updateArcs = function () {
			...
			//filter parent paths
			paths.parents = $$.selection.arcs.arc.path.filter(function (node) {
				return sequence.indexOf(node) >= 0;
			...
			//filter child paths
			paths.children = $$.selection.arcs.arc.path.filter(function (node) {
				return sequence.indexOf(node) < 0;
			...
			$$.selection.arcs.arc.exit().transition().duration($$.animationDuration * 1.5).style('opacity', 0).remove().select('path').each(function (d) {
				this.newArc = {
			...
			var pathTransition = $$.selection.arcs.arc.path.transition().duration($$.animationDuration * 1.5).call(d2b.UTILS.TWEENS.arcTween, $$.arc);
			// .attrTween("d", $$.arcTween);

			//update oldArc to be newArc
			pathTransition.each("end", function (d) {
		...
		$$.setupNewData = function () {
			...
			var topNodes = $$.partitionData.filter(function (d) {
				return d.top;
			});
			if ($$.controlsData.hideLegend.enabled) {
				$$.legendData = { data: { items: [] } };
				...
				$$.legendData = {
					data: {
						items: d3.set(topNodes.map(function (d) {
							return d.name;
						})).values().map(function (d) {
							return { label: d };
						})
		...
		$$.updateBreadcrumbs = function (sequence) {
			var breadcrumbsData = {
				data: {
					items: sequence.map(function (d, i) {
						return { label: d.name, key: i + ',' + d.name, data: d };
					})
				}
			};
			$$.breadcrumbs.data(breadcrumbsData).update();
			...
			// .attr('stroke-width',2)
			.attr('stroke-width', 1.5).style('fill-opacity', 0.2).style('stroke-opacity', 0.7).style('fill', function (d) {
				return $$.arcFill(d.data);
			}).style('stroke', function (d) {
				return d3.rgb($$.arcFill(d.data)).darker(1);
			});
		};
		...
		chart.generate = function (callback) {
			...
			$$.selection.main = $$.selection.group.append('g').attr('class', 'd2b-sunburst').on('mouseout.d2b-mouseout', $$.sunburstMouseout);

			$$.selection.arcs = $$.selection.main.append('g').attr('class', 'd2b-sunburst-arcs');

			$$.selection.tooltip = $$.selection.main.append('g').attr('class', 'd2b-sunburst-tooltip');

			$$.selection.tooltip.text = $$.selection.tooltip.append('text');

			//create breadcrumbs container
			$$.selection.breadcrumbs = $$.selection.group.append('g').attr('class', 'd2b-sunburst-breadcrumbs');
			...
			$$.breadcrumbs.selection($$.selection.breadcrumbs);
		...
		chart.update = function (callback) {
			...
			$$.selection.breadcrumbs.transition().duration($$.animationDuration).attr('transform', 'translate(' + $$.forcedMargin.left + ',' + 0 + ')');

			$$.breadcrumbs.width($$.innerWidth).update();

			d2b.UTILS.CHARTS.HELPERS.updateLegend($$);

			$$.selection.main.transition().duration($$.animationDuration).attr('transform', 'translate(' + ($$.forcedMargin.left + $$.innerWidth / 2) + ',' + ($$.forcedMargin.top + $$.innerHeight / 2) + ')');
			...
			$$.selection.arcs.arc = $$.selection.arcs.selectAll("g.d2b-sunburst-arc").data($$.partitionData, function (d, i) {
				if (d.key == 'unique') return Math.floor((1 + Math.random()) * 0x10000);else if (d.key && d.key != 'auto') return d.key;else return $$.getAncestors(d).map(function (d) {
					return d.name;
				}).join('-');
			});
			// sunburst_mouseout();
			var newArcs = $$.selection.arcs.arc.enter().append("g").attr('class', 'd2b-sunburst-arc').style('opacity', 0).call($$.events.addElementDispatcher, 'main', 'd2b-arc');

			var newPaths = newArcs.append("path").on('mouseover.d2b-mouseover', $$.arcMouseover);

			$$.selection.arcs.arc.transition().duration($$.animationDuration).style('opacity', '');

			$$.selection.arcs.arc.path = $$.selection.arcs.arc.select('path').style('fill', $$.arcFill);

			$$.selection.arcs.arc.path.on('click.d2b-click', $$.arcClick).classed('d2b-pointer-element', true);
	...
	d2b.CHARTS.tableChart = function () {
		...
		$$.updateSubFacts = function (row, subFacts, subFactsData, x, padding, height, showValueLabels) {
			...
			var newSubFact = subFact.enter().append('g').attr('class', 'd2b-table-subFact');
			...
			subFact.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();

			return subFact;
		};

		$$.updateSubFact = function (subFact, x, padding, height, showValueLabels) {
			subFact.select('text.d2b-table-subFact-label').text(function (d) {
				return d.label;
			}).transition().duration($$.animationDuration).attr('y', -4).attr('x', (x.rangeBand() - padding) / 2);

			subFact.each(function (d) {

				d.x = d3.scale.linear();

				d.x.domain([0, d3.sum(d.values, function (d) {
					return d.value;
			...
			var subFactValue = subFact.select('g').selectAll('g').data(function (d) {
				return d.values;
			});

			var newSubFactValue = subFactValue.enter().append('g').call($$.events.addElementDispatcher, 'main', 'd2b-table-row-sub-fact');

			newSubFactValue.append('rect');
			newSubFactValue.append('text').attr('class', 'd2b-subFact-value-label');

			var subFactValueTransition = subFactValue.call(d2b.UTILS.bindTooltip, $$.tooltip, function (d) {
				return d;
			}).transition().duration($$.animationDuration);

			subFactValueTransition.select('rect').attr('x', function (d) {
				return d.xPos;
			}).attr('width', function (d) {
				return d.width;
			}).attr('height', height).style('fill', d2b.UTILS.getColor($$.color, 'label'));

			subFactValueTransition.select('text.d2b-subFact-value-label').style('opacity', +showValueLabels).attr('y', height / 2 + 4).attr('x', function (d) {
				return d.xPos + d.width / 2;
			}).text(function (d) {
				return d.label.substring(0, d.width / 10);
			});

			subFactValue.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
		};
		...
		$$.showRow = function () {
			...
			$$.selection.table.row.select('tspan.d2b-label').text(function (d) {
				return d.label + ' ';
			});
			$$.selection.table.row.select('tspan.d2b-value').text(function (d) {
				return $$.xFormat(d.value);
			});
			...
			var rowTransition = $$.selectedRow.transition().duration($$.animationDuration).style('opacity', 1).attr('transform', function (d, i) {
				return 'translate(0,' + $$.forcedMargin.top + ')';
			});

			rowTransition.select('rect.d2b-row-border').attr('width', $$.innerWidth);

			rowTransition.select('rect.d2b-row-background').attr('height', $$.innerHeight);

			var subFacts = $$.selectedRow.select('.d2b-table-subFacts');

			var x = d3.scale.ordinal();

			x.domain([0]).rangeBands([0, $$.innerWidth]);

			var y = d3.scale.ordinal();

			y.domain(d3.range(0, $$.selectedRow.data.subFacts.length)).rangeBands([5, $$.innerHeight]);
			$$.updateSubFacts($$.selectedRow, subFacts, $$.selectedRow.data.subFacts, x, 0, y.rangeBand() / 2, true).transition().duration($$.animationDuration).attr('transform', function (d, i) {
				return 'translate(0,' + (y(i) + y.rangeBand() / 2) + ')';
			});
		};
		...
		$$.showTable = function () {
			...
			$$.selection.table.row = $$.selection.table.selectAll('g.d2b-table-row').data($$.currentChartData.rows);
			var newTableRow = $$.selection.table.row.enter().append('g').call($$.events.addElementDispatcher, 'main', 'd2b-table-row').attr('class', 'd2b-table-row');

			newTableRow.append('rect').attr('class', 'd2b-row-background');

			newTableRow.append('rect').attr('class', 'd2b-row-border');

			var newTableRowText = newTableRow.append('text').attr('class', 'd2b-table-row-label');
			...
			var rowTransition = $$.selection.table.row.transition().duration($$.animationDuration).style('opacity', 1).attr('transform', function (d, i) {
				return 'translate(0,' + (i * rowHeight + rowHeight / 2) + ')';
			});

			rowTransition.select('rect.d2b-row-border').attr('width', $$.innerWidth).attr('height', 1.5).attr('y', 5);

			rowTransition.select('rect.d2b-row-background').attr('width', $$.innerWidth).attr('height', 30).attr('y', -22);
			...
			$$.selection.table.row.select('text.d2b-table-row-label').each(function () {
				longestText = Math.max(this.getComputedTextLength(), longestText);
			});

			$$.selection.table.row.style('pointer-events', 'all').each(function (d) {
				...
				if (d.subFacts.length > 0) {
					text.transition().duration($$.animationDuration).style('text-anchor', 'start').attr('x', 0);
					row.style('cursor', 'pointer');
				} else {
					text.transition().duration($$.animationDuration).style('text-anchor', 'middle').attr('x', $$.innerWidth / 2);
					row.style('cursor', 'auto');
				}

				x.domain(d3.range(0, d.subFacts.length)).rangeBands([longestText + padding, $$.innerWidth + padding]);

				$$.updateSubFacts(row, subFacts, d.subFacts, x, padding, 5, false).transition().duration($$.animationDuration).attr('transform', function (d, i) {
					return 'translate(' + x(i) + ',-2)';
				});
				...
				if ($$.selectedRow) {
					...
					if (d.subFacts.length > 0) {
						$$.selection.table.row.style('pointer-events', 'none').transition().duration($$.animationDuration).style('opacity', 0);
		...
		chart.generate = function (callback) {
			...
			//init main chart container
			$$.selection.main = $$.selection.group.append('g').attr('class', 'd2b-table-chart');

			$$.selection.table = $$.selection.main.append('g').attr('class', 'd2b-table');
   ...
   */
     ...
     */
			addDispatcher: function addDispatcher(events) {
				...
				$$[store] = d3.dispatch.apply(_this2, events);
   ...
   */
	...
	d2b.model.chart = function (update) {
		...
		var chart = function chart(selection) {
			...
			chart.selection(selection).data(selection.datum()).update();
		};

		/* Inherit from base model */
		var model = d2b.model.base(chart, $$).addProp('selection', d3.select('body')).addProp('size', null).addProp('duration', 500).addProp('legendAt', 'center right').addProp('padding', { top: 0, left: 0, right: 0, bottom: 0 }, function (_) {
			...
			return chart;
		}).addPropGet('checkbox', d2b.svg.checkbox().on('change.d2b-chart-checkbox', function () {
			return chart.update();
		})).addPropGet('legend', d2b.svg.legend()).addMethod('select', function (_) {
			$$.selection = d3.select(_);
			return chart;
		}).addMethod('update', function (callback) {
			...
			chart.duration(0).update().duration(duration);
		}).addMethod('control', function (key, data) {
			if (arguments.length === 0) return controlsData;
			var control = controlsData.filter(function (d) {
				return d.key === key;
			})[0];
			if (!control) {
			...
			for (var _key in data) {
				if (_key !== 'key') control[_key] = data[_key];
			}return chart;
		...
		var updateControls = function updateControls(transition) {
			var margin = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];


			$$.svg.controls = $$.svg.group.selectAll('.d2b-controls').data([controlsData.filter(function (d) {
				return d.visible;
			})]);

			var newControls = $$.svg.controls.enter().append('g').attr('class', 'd2b-controls');

			var control = $$.svg.controls.selectAll('.d2b-control').data(function (d) {
				return d;
			}, function (d) {
				return d.key;
			});

			var newControl = control.enter().append('g').style('opacity', 0).attr('class', 'd2b-control');

			control.exit().transition().duration($$.duration).style('opacity', 0).remove();
			...
			control.call($$.checkbox).each(function (d) {
			...
			newControl.attr('transform', function () {
				return this.translate;
			});

			newControls.attr('transform', function () {
				return this.translate;
			});

			transition.selectAll('.d2b-control').style('opacity', 1).attr('transform', function () {
				return this.translate;
			});

			var marginTop = y + boxHeight;
		...
		/* Legend */
		var updateLegend = function updateLegend(transition) {
			var margin = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];


			$$.svg.legend = $$.svg.group.selectAll('.d2b-legend').data([$$.legend]);
			var newLegend = $$.svg.legend.enter().append('g').attr('class', 'd2b-legend');

			var size = undefined,
			    x = undefined,
			    y = undefined;
			var at = $$.legendAt.split(" ");
			at = { x: at[1], y: at[0] };
			var legendTransition = $$.svg.legend.transition().duration($$.duration);

			$$.legend.selection($$.svg.legend).duration($$.duration).maxSize({ width: $$.width, height: $$.height }).update();
			...
			if (at.x !== 'center' || at.y !== 'center') {
				if ($$.legend.orient() === 'horizontal') {
					if (at.y === 'top') margin.top += size.height;else if (at.y === 'bottom') margin.bottom += size.height;
					$$.height -= size.height;
				} else {
					if (at.x === 'left') margin.left += size.width;else if (at.x === 'right') margin.right += size.width;
					$$.width -= size.width;
				}
			}
		...
		/* Main Container */
		var updateContainer = function updateContainer(transition) {
			var margin = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];

			$$.svg.main = $$.svg.group.selectAll('.d2b-main').data([$$.data]);
			$$.svg.main.enter().append('g').attr('class', 'd2b-main').attr('transform', "translate(" + margin.left + ", " + margin.top + ")");

			transition.select('.d2b-main').attr('transform', "translate(" + margin.left + ", " + margin.top + ")");
		};
		...
		model.build = function () {
			...
			var newSvg = $$.svg.enter().append('svg').attr('class', 'd2b-svg');

			newSvg.append('g').attr('class', 'd2b-group').attr('transform', "translate(" + $$.padding.left + ", " + $$.padding.top + ")");
			...
			var transition = $$.svg.transition().duration($$.duration).attr('width', $$.width).attr('height', $$.height).select('.d2b-group').attr('transform', "translate(" + $$.padding.left + ", " + $$.padding.top + ")");
	...
	d2b.SVG.axis = function () {
		...
		$$.getMaxTickBox = function (axis) {
			...
			var test = d3.select('body').append('svg').attr('class', 'd2b-axis').style('opacity', 0);

			var maxTickBox = { width: 0, height: 0 };
			test.call(axis).selectAll('.tick text').each(function (d) {
				var box = this.getBBox();
		...
		$$.getLabelHeight = function (classAlt) {
			...
			var testSvg = d3.select('body').append('svg').attr('class', 'd2b-axis');
			var testText = testSvg.append('text').attr('class', 'd2b-axis-label ' + classAlt).text('Test').style('opacity', 0);
		...
		$$.denoteOriginTicks = function () {
			...
			this.selectAll('.tick text').each(function (d) {
				if (parseFloat(this.textContent) === 0) d3.select(this.parentNode).classed('d2b-origin-tick', true);else d3.select(this.parentNode).classed('d2b-origin-tick', false);
			});
		};
		...
		$$.updateGrid = function (d) {
			...
			transition.call(grid).call($$.denoteOriginTicks).style('opacity', 1);
		};
		...
		$$.updateAxis = function (d) {
			...
			transition.call(_axis).call($$.denoteOriginTicks).style('opacity', 1).attr('transform', transform);
		};
		...
		$$.updateLabelInside = function (d) {
			...
			transition.attr('transform', transform).text(d.labelInside || '');
		};
		...
		$$.updateLabelOutside = function (d) {
			...
			transition.attr('transform', transform).text(d.labelOutside || '');
		};
		...
		$$.updateTickCounts = function (width, height) {
			...
			$$.axesVisible.forEach(function (_axis) {
				...
				if (_axis.axis.scale().rangeBand) {} else if (_axis.tickCount) {
		...
		var axis = function axis(g) {
			...
			g.each(function (d, i) {
				...
				g._enter = g.enter().append('g').attr('class', 'd2b-axis-container');
				g._update = d3.transition(g).attr('transform', 'translate(' + $$.padding.left + ',' + $$.padding.top + ')');

				//make grid
				g.grid = g.selectAll('g.d2b-axis-grid').data($$.axesVisible, function (d) {
					return d.key;
				});
				g.grid._enter = g.grid.enter().append('g').attr('class', function (d) {
					return 'd2b-axis-grid d2b-' + d.key;
				}).style('opacity', 0).each($$.updateGrid);
				g.grid._update = d3.transition(g.grid).each($$.updateGrid);
				g.grid._exit = d3.transition(g.grid.exit()).style('opacity', 0).remove();

				//make axes
				g.axis = g.selectAll('g.d2b-axis').data($$.axesVisible, function (d) {
					return d.key;
				});
				g.axis._enter = g.axis.enter().append('g').style('opacity', 0).attr('class', function (d) {
					return 'd2b-axis d2b-' + d.key;
				}).each($$.updateAxis);
				g.axis._update = d3.transition(g.axis).each($$.updateAxis);
				g.axis._exit = d3.transition(g.axis.exit()).style('opacity', 0).remove();
		...
		axis.scale = function (_axisKey) {
			...
			if ($$.axisKeys.indexOf(_axisKey) > -1) {
				_scale2 = function scale(value) {
					var _side = axis[_axisKey]().orientation.split('-')[0];
					var position, value;
					if ((typeof value === "undefined" ? "undefined" : _typeof(value)) === 'object' && _axisKey in value) {
					...
					if (_reversed) {
						position = d3.max(_range) - position + d3.min(_range) + _scale2.rangeBand() / 2;
					} else {
						position += _scale2.rangeBand() / 2;
					}

					return position;
				};
			} else {
				_scale2 = function _scale2() {
					return null;
				};
			};

			_scale2.range = function () {
				var range = [];
				if (_scale.rangeBand) {
					range = _scale.range();
			...
			_scale2.reversed = function (reversed) {
				_reversed = reversed;
				return _scale2;
			};

			_scale2.invert = function (value) {
				// todo: support invert for ordinal/reversed scales
			...
			_scale2.change = function (_newAxisKey) {
	...
	d2b.SVG.point = function () {
		...
		$$.mouseover = function (d, i) {
			...
			d3.select(this).select('path.d2b-point-background').transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).attr("d", $$.symbol);
		};
		...
		var point = function point(g) {
			g.each(function (d, i) {
				...
				backgroundEnter.attr('d', $$.symbol).style('fill', 'rgba(255,255,255,0)').style('stroke-width', strokeWidth).style('stroke', stroke);

				backgroundUpdate.attr('d', $$.symbol).style('stroke', stroke);
				...
				foregroundEnter.attr('d', $$.symbol).style('stroke-width', strokeWidth).style('stroke', stroke).style('fill', fill);

				foregroundUpdate.attr('d', $$.symbol).style('fill', fill).style('stroke', stroke);

				foregroundExit.style('opacity', 0).remove();

				//if active set hover events
				if (active) {
					g.on('mouseover.d2b-mouseover', $$.mouseover).on('mouseout.d2b-mouseout', $$.mouseout);
				} else {
					g.on('mouseover.d2b-mouseover', null).on('mouseout.d2b-mouseout', null);
				}
			});
		};
	...
	d2b.SVG.symbol = function () {
		...
		var symbol = function symbol(d, i) {
			...
			if (d3.svg.symbolTypes.indexOf(type) > -1) $$.symbol = $$.d3Symbol.type(type).size(size);else if (d2b.SVG.symbolTypes.indexOf(type) > -1) $$.symbol = d2b.SVG.symbols[type];else $$.symbol = $$.d3Symbol.type('circle').size(size);
	...
	d2b.SVG.tooltip = function () {
		...
		$$.refreshLayout['0'] = function () {
			for (var key in $$.graph) {
				$$.graph[key].call(d2b.UTILS.bindTooltip, $$.graph[key].config.content, $$.graph[key].config.data, $$.graph[key].config.fill);
			}
		};
		...
		$$.refreshLayout['1'] = function () {
			...
			var tooltip = $$.container.selectAll('.d2b-tooltip').data(graphArray, function (d) {
				return d.config.key;
			});
		...
		$$.mousemoveLayout1 = function (event, tooltip) {
			...
			tooltip.each(function (tooltipData) {
				...
				//set tooltip text
				text.text(this.closest.content);
				...
				this.offsetY = $$.offsetForCollisionResolution(verticalPixelCells, $$.y(this.closest.y), textBox.height + paddingY * 2 + marginY * 2);

				//position tooltip svg:g
				elem.update.attr('transform', 'translate(' + $$.x(this.closest.x) + ',' + $$.y(this.closest.y) + ')');

				rect.attr('width', textBox.width + paddingX * 2).attr('height', textBox.height + paddingY * 2).style('stroke', this.closest.darkFill).style('fill', this.closest.fill);
				circle.style('stroke', this.closest.darkFill);
				line.style('stroke', this.closest.darkFill);

				if (position === 'right') {
					text.update.attr('transform', 'translate(' + (lineLength + paddingX) + ',' + (textBox.height / 3.5 + this.offsetY) + ')').style('text-anchor', 'start');
					rect.update.attr('transform', 'translate(' + lineLength + ',' + (-textBox.height / 2 - paddingY + this.offsetY) + ')');
					line.update.attr('points', '0,0 0,' + this.offsetY + ' ' + lineLength + ',' + this.offsetY);
				} else {
					text.update.attr('transform', 'translate(' + (-lineLength - paddingX) + ',' + (textBox.height / 3.5 + this.offsetY) + ')').style('text-anchor', 'end');
					rect.update.attr('transform', 'translate(' + (-textBox.width - lineLength - paddingX * 2) + ',' + (-textBox.height / 2 - paddingY + this.offsetY) + ')');
					line.update.attr('points', '0,0 0,' + this.offsetY + ' ' + -lineLength + ',' + this.offsetY);
				}
			});
		};
		...
		$$.getClosestProperties = function (point, nodes) {
			var smallestDistX = Infinity,
			    smallestDistY = Infinity;

			var closest = null;

			nodes.each(function (d, i) {
				var node = {};
				node.x = typeof nodes.config.x === "function" ? nodes.config.x.apply(this, arguments) : nodes.config.x || null;
				node.y = typeof nodes.config.y === "function" ? nodes.config.y.apply(this, arguments) : nodes.config.y || null;
				...
				if (distX > smallestDistX) {
					return;
				}
				if (distX === smallestDistX && distY > smallestDistY) {
					return;
				}

				node.point = [node.x, node.y];
				node.data = typeof nodes.config.data === "function" ? nodes.config.data.apply(this, arguments) : nodes.config.data || d;
				node.content = typeof nodes.config.content === "function" ? nodes.config.content.apply(this, [node.data, i]) : d.config.content;
				node.fill = typeof nodes.config.fill === "function" ? nodes.config.fill.apply(this, arguments) : nodes.config.fill || d;
		...
		$$.stop = function () {
			...
			for (var key in $$.graph) {
				$$.graph[key].call(d2b.UTILS.unbindTooltip);
   ...
   */
	...
	d2b.UTILS.breadcrumbs = function () {
		...
		breadcrumbs.on = function (key, value) {
			key = key.split('.');
			if (!arguments.length) return on;else if (arguments.length == 1) {
				if (key[1]) return on[key[0]][key[1]];else return on[key[0]]['default'];
			};

			if (key[1]) on[key[0]][key[1]] = value;else on[key[0]]['default'] = value;
		...
		breadcrumbs.update = function (callback) {
			if (!selection) return console.warn('breadcrumbs was not given a selection');

			if (callback) {
				callback;
			}

			selection.breadcrumb = selection.selectAll('g.d2b-breadcrumb').data(currentBreadcrumbsData.items, function (d) {
				return d.key ? d.key : i;
			});

			var newBreadcrumbs = selection.breadcrumb.enter().append('g').attr('class', 'd2b-breadcrumb').style('opacity', 0);
			...
			selection.breadcrumb.text = selection.breadcrumb.select('text').text(function (d) {
				return d.label;
			}).attr('x', function (d, i) {
				if (i == 0) return padding + breadcrumbIndentSize / 2;else return padding + breadcrumbIndentSize;
			}).attr('y', scale * 2.9).style('font-size', scale * 2.5 + 'px');
			...
			selection.breadcrumb.each(function (d, i) {
				...
				bc.path.transition().duration(animationDuration / 2).attr('d', 'M 0 0 L ' + leftIndent + ' ' + breadcrumbHeight / 2 + ' L 0 ' + breadcrumbHeight + ' L ' + pathWidth + ' ' + breadcrumbHeight + ' L ' + (pathWidth + rightIndent) + ' ' + breadcrumbHeight / 2 + ' L ' + pathWidth + ' 0 L 0 0 Z');

				bc.transition().duration(animationDuration / 2).style('opacity', 1).attr('transform', 'translate(' + bcOffset + ',0)');
			...
			selection.breadcrumb.exit().transition().duration(animationDuration / 4).style('opacity', 0).remove();
	...
	d2b.UTILS.chartAdapter = function (type, chartData) {
		...
		if (chartData.properties) {
			for (var key in chartData.properties) {
				if (chartData.properties[key].args) chartData.chart[key].apply(this, chartData.properties[key].args);else chartData.chart[key](chartData.properties[key]);
			}
		}
	};
	...
	d2b.UTILS.chartLayoutAdapter = function (type, chartData) {
		chartData.chart = new d2b.CHARTS[type]();
		if (!chartData.chartLayoutData) chartData.chartLayoutData = {};
		chartData.chartLayout = new d2b.UTILS.CHARTPAGE.chartLayout();
		chartData.chartLayout.chart(chartData.chart).data(chartData.chartLayoutData);
	...
	d2b.UTILS.CHARTS.HELPERS.updateLegend = function (_chart) {
		...
		_chart.legend.orientation(_chart.legendOrientation == 'right' || _chart.legendOrientation == 'left' ? "vertical" : "horizontal").data(_chart.legendData).height(_chart.innerHeight).width(_chart.innerWidth).update();

		var legendTranslation;
		if (_chart.legendOrientation == 'right') legendTranslation = 'translate(' + (_chart.forcedMargin.left + _chart.innerWidth - _chart.legend.computedWidth()) + ',' + ((_chart.innerHeight - _chart.legend.computedHeight()) / 2 + _chart.forcedMargin.top) + ')';else if (_chart.legendOrientation == 'left') legendTranslation = 'translate(' + _chart.forcedMargin.left + ',' + ((_chart.innerHeight - _chart.legend.computedHeight()) / 2 + _chart.forcedMargin.top) + ')';else if (_chart.legendOrientation == 'top') legendTranslation = 'translate(' + (_chart.forcedMargin.left + (_chart.innerWidth - _chart.legend.computedWidth()) / 2) + ',' + _chart.forcedMargin.top + ')';else legendTranslation = 'translate(' + (_chart.forcedMargin.left + (_chart.innerWidth - _chart.legend.computedWidth()) / 2) + ',' + (_chart.innerHeight + _chart.forcedMargin.top - _chart.legend.computedHeight()) + ')';

		_chart.selection.legend.transition().duration(_chart.animationDuration).attr('transform', legendTranslation);
		...
		if (computedSize) computedSize += legendPadding;
	...
	d2b.UTILS.CHARTS.HELPERS.updateControls = function (_chart) {
		...
		var controlsData = d2b.UTILS.getValues(_chart.controlsData).filter(function (d) {
			return d.visible;
		});
		...
		_chart.selection.controls.transition().duration(_chart.animationDuration).attr('transform', 'translate(' + (_chart.forcedMargin.left + _chart.innerWidth - _chart.controls.computedWidth()) + ',' + _chart.forcedMargin.top + ')');

		var computedSize = _chart.controls.computedHeight();
		if (computedSize) computedSize += controlsPadding;
	...
	d2b.UTILS.CHARTS.HELPERS.generateDefaultSVG = function (_chart) {
		...
		//create svg
		_chart.selection.svg = _chart.selection.append('svg').attr('class', 'd2b-svg d2b-container');

		//create group container
		_chart.forcedMargin = d2b.CONSTANTS.DEFAULTFORCEDMARGIN();
		_chart.selection.group = _chart.selection.svg.append('g').attr('transform', 'translate(' + _chart.forcedMargin.left + ',' + _chart.forcedMargin.top + ')');

		//create legend container
		_chart.selection.legend = _chart.selection.group.append('g').attr('class', 'd2b-legend');

		//create controls container
		_chart.selection.controls = _chart.selection.group.append('g').attr('class', 'd2b-controls');
	};
	...
	d2b.UTILS.CHARTPAGE.chartLayout = function () {
		...
		chartLayout.data = function (chartLayoutData, reset) {
			...
			if (chartLayoutData.data.chartLayout.footnote) currentChartLayoutData.chartLayout.footnote = chartLayoutData.data.chartLayout.footnote;

			if (chartLayoutData.data.chartLayout.rightNotes) currentChartLayoutData.chartLayout.rightNotes = chartLayoutData.data.chartLayout.rightNotes;

			if (chartLayoutData.data.chartLayout.leftNotes) currentChartLayoutData.chartLayout.leftNotes = chartLayoutData.data.chartLayout.leftNotes;

			if (chartLayoutData.data.chartLayout.title) currentChartLayoutData.chartLayout.title = chartLayoutData.data.chartLayout.title;

			if (chartLayoutData.data.chartLayout.titleAlt) currentChartLayoutData.chartLayout.titleAlt = chartLayoutData.data.chartLayout.titleAlt;

			if (chartLayoutData.data.chartCallback) currentChartLayoutData.chartCallback = chartLayoutData.data.chartCallback;

			if (chartLayoutData.data.chartLayout.info) currentChartLayoutData.chartLayout.info = chartLayoutData.data.chartLayout.info;
		...
		chartLayout.generate = function (callback) {
			...
			selection.wrapper = selection.append('div').attr('class', 'd2b-chart-layout-wrapper');

			selection.container = selection.wrapper.append('div').attr('class', 'd2b-chart-layout d2b-container');

			selection.container.header = selection.container.append('div').attr('class', 'd2b-chart-layout-header');

			selection.container.header.title = selection.container.header.append('div').attr('class', 'd2b-chart-layout-title');

			selection.container.header.titleAlt = selection.container.header.append('div').attr('class', 'd2b-chart-layout-title-alt');

			selection.container.header.infoIcon = selection.container.header.append('div').attr('class', 'd2b-chart-layout-info-icon');

			selection.container.info = selection.container.append('div').attr('class', 'd2b-chart-layout-info');

			selection.container.chart = selection.container.append('div').attr('class', 'd2b-chart-layout-chart');

			chart.selection(selection.container.chart);

			selection.container.rightNotes = selection.container.append('div').attr('class', 'd2b-chart-layout-right-notes');

			selection.container.rightNotes.ul = selection.container.rightNotes.append('ul');

			selection.container.leftNotes = selection.container.append('div').attr('class', 'd2b-chart-layout-left-notes');

			selection.container.leftNotes.ul = selection.container.leftNotes.append('ul');

			selection.container.footnote = selection.container.append('div').attr('class', 'd2b-chart-layout-footnote');
			selection.container.footnote.div = selection.container.footnote.append('div');

			selection.container.source = selection.container.append('div').attr('class', 'd2b-chart-layout-source').append('ul');

			generateRequired = false;

			var tempAnimationDuration = animationDuration;
			chartLayout.animationDuration(0).update(callback).animationDuration(tempAnimationDuration);

			return chartLayout;
		};

		chartLayout.update = function (callback) {
			if (!selection) return console.warn('chartLayout was not given a selection');
			if (!chart) return console.warn('chartLayout was not given a chart');
			...
			selection.wrapper.style('width', width + 'px').style('height', height + 'px');
			selection.container.header.title.text(currentChartLayoutData.chartLayout.title);
			selection.container.header.titleAlt.text(currentChartLayoutData.chartLayout.titleAlt);
			...
			if (currentChartLayoutData.chartLayout.info) {
				selection.container.header.infoIcon.html('<i class="fa fa-info-circle"></i>').on('click', function () {
					selection.container.info.classed('d2b-visible', !selection.container.info.classed('d2b-visible'));
				});
				selection.container.info.html(currentChartLayoutData.chartLayout.info).style('top', headerHeight + 'px').style('height', height - headerHeight + 'px');
			} else {
				selection.container.header.infoIcon.selectAll('*').on('click.d2b-click', null).remove();
				selection.container.info.selectAll('*').remove();
			}
			...
			selection.container.footnote.style('top', height - chartMargin.bottom + 'px');
			...
			rightNote.enter().append('li').attr('class', 'd2b-chart-layout-note').append('div');

			var rightNotesHeight = 0;
			rightNote.select('div').text(function (d) {
				return d;
			}).each(function (d) {
				rightNotesHeight += this.getBoundingClientRect().height;
			});

			rightNote.exit().style('opacity', 0).remove();

			// var rightNotesHeight = selection.container.rightNotes.ul.node().getBoundingClientRect().height;
			selection.container.rightNotes.style('width', width * 0.2 + 'px').style('height', height - chartMargin.top + 'px').style('left', width * 0.8 - 5 + 'px');
			// .style('top',(height-rightNotesHeight)/2+'px');

			rightNote.style('padding-top', Math.max(10, (height - rightNotesHeight) / (1.8 * rightNote.size())) + 'px');
			...
			leftNote.enter().append('li').attr('class', 'd2b-chart-layout-note').append('div');

			var leftNotesHeight = 0;
			leftNote.select('div').text(function (d) {
				return d;
			}).each(function (d) {
				leftNotesHeight += this.getBoundingClientRect().height;
			});

			leftNote.exit().style('opacity', 0).remove();

			selection.container.leftNotes.style('width', width * 0.2 + 'px').style('left', 5 + 'px');
			// .style('height',leftNotesHeight+'px')
			// .style('top',(height-leftNotesHeight)/2+'px');

			leftNote.style('padding-top', Math.max(10, (height - leftNotesHeight) / (1.8 * leftNote.size())) + 'px');
			...
			selection.container.chart.style('left', chartMargin.left + 5 + 'px').style('top', chartMargin.top + 'px').style('width', chartWidth + 'px').style('height', chartHeight + 'px');

			chart.width(chartWidth).height(chartHeight).animationDuration(animationDuration).update(currentChartLayoutData.chartCallback);
	...
	d2b.UTILS.CONTROLS.checkbox = function () {
		...
		control.update = function (callback) {
			...
			var newCheckboxContainer = checkboxContainer.enter().append('g').attr('class', 'd2b-checkbox-container').on('click.d2b-click', function (d, i) {
				$$.currentData.state = !$$.currentData.state;
				$$.events.dispatch("change", this, [d, i]);
				control.update();
			}).call($$.events.addElementDispatcher, 'main', 'd2b-checkbox');

			newCheckboxContainer.append('rect').attr('rx', 1);
			newCheckboxContainer.append('path');

			newCheckboxContainer.append('text').text(function (d) {
				return d.label;
			});

			var checkboxTranslate = 'translate(' + 0.4 * $$.scale + ',' + 0.4 * $$.scale + ')';
			var check = checkboxContainer.select('path').attr('transform', checkboxTranslate).attr('d', "M" + 0.38 * $$.scale + "," + 1.06 * $$.scale + " l" + 0.7 * $$.scale + "," + 0.5 * $$.scale + " l" + 0.58 * $$.scale + "," + -1.19 * $$.scale + "").style('stroke-width', 0.25 * $$.scale).attr('stroke-dasharray', 2.2 * $$.scale);
			check.transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).attr('stroke-dashoffset', $$.currentData.state ? 0 : 2.2 * $$.scale);
			var box = checkboxContainer.select('rect').attr('width', $$.scale * 2.1 + 'px').attr('height', $$.scale * 2.1 + 'px').attr('transform', checkboxTranslate).style('stroke-width', 0.25 * $$.scale);

			var label = checkboxContainer.select('text').attr('y', $$.scale * 2.4).attr('x', $$.scale * 3.2).style('font-size', $$.scale * 2.5 + 'px');
			...
			if ($$.container) {
				if ($$.container.svg) {
					$$.container.svg.attr('width', $$.computedWidth).attr('height', $$.computedHeight);
				}
			}
	...
	d2b.UTILS.CONTROLS.radioButtons = function () {
		...
		control.data = function (data, reset) {
			...
			$$.currentData.values.forEach(function (d) {
				if (foundSelected) d.selected = false;
		...
		control.update = function (callback) {
			...
			var radioButtonContainer = $$.selection.selectAll('g.d2b-radio-button-container').data($$.currentData.values, function (d) {
				return d.label;
			});

			var newRadioButtonContainer = radioButtonContainer.enter().append('g').attr('class', 'd2b-radio-button-container').on('click.d2b-click', function (d, i) {
				$$.resetValues();
				d.selected = true;
				$$.currentData.selected = d;
			...
			radioButtonContainer.attr('transform', function (d, i) {
				return 'translate(' + 0 + ',' + i * 3.3 * $$.scale + ')';
			});

			newRadioButtonContainer.append('circle').attr('class', 'd2b-radio-button-inner').style('fill-opacity', 0);
			newRadioButtonContainer.append('circle').attr('class', 'd2b-radio-button-outer');

			newRadioButtonContainer.append('text').text(function (d) {
				return d.label;
			});

			var circleInner = radioButtonContainer.select('circle.d2b-radio-button-inner').attr('cy', $$.scale * 1.4).attr('cx', $$.scale * 1.4);

			var circleOuter = radioButtonContainer.select('circle.d2b-radio-button-outer').attr('r', $$.scale * 1 + 'px').attr('cy', $$.scale * 1.4).attr('cx', $$.scale * 1.4).style('stroke-width', 0.25 * $$.scale);

			circleInner.transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).style('fill-opacity', function (d) {
				return d.selected ? 1 : 0;
			}).attr('r', function (d) {
				return d.selected ? $$.scale * 0.5 : 0;
			});

			var label = radioButtonContainer.select('text').attr('y', $$.scale * 2.4).attr('x', $$.scale * 3.2).style('font-size', $$.scale * 2.5 + 'px');
	...
	d2b.UTILS.CONTROLS.htmlControls = function () {
		...
		controls.update = function (callback) {
			if (!$$.selection) return console.warn('controls was not given a selection');
			// console.log($$.currentData.controls)
			$$.selection.controlContainer = $$.selection.selectAll('div.d2b-control-container').data($$.currentData.controls, function (d) {
				return d.type + '-' + d.key;
			});

			var newControl = $$.selection.controlContainer.enter().append('div').attr('class', 'd2b-control-container');

			newControl.append('div').attr('class', 'd2b-control-label').text(function (d) {
				return d.label + ':';
			});

			newControl.append('div').attr('class', 'd2b-control').each(function (d) {
				this.control = new d2b.UTILS.CONTROLS[d.type]();
				this.control.container(d3.select(this));
			});

			$$.selection.controlContainer.control = $$.selection.controlContainer.select('.d2b-control').each(function (d) {
				this.control.scale($$.scale).data(d).on('change.d2b-change', function (d, i) {
	...
	d2b.UTILS.CONTROLS.controls = function () {
		...
		controls.update = function (callback) {
			...
			if ($$.currentData.length > 0) {
				var controls = $$.selection.selectAll('g.d2b-control').data($$.currentData, function (d) {
					return d.label + ',' + d.type;
				});

				controls.enter().append('g').attr('class', 'd2b-control').each(function (d) {
				...
				controls.each(function (d) {
					...
					d3.select(this).transition().duration($$.animationDuration).style('opacity', 1).attr('transform', 'translate(' + $$.computedWidth + ',' + $$.computedHeight + ')');
				...
				controls.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
			}
	...
	d2b.UTILS.customEvents = function () {
		...
		return {
			...
			dispatch: function dispatch(key, my, args) {
				// dispatch all listeners with the event key
				for (var listener in $$.events[key]) {
					if ($$.events[key][listener]) $$.events[key][listener].apply(my, args);
				}
			},
	...
	d2b.UTILS.applyProperties = function (properties) {
		for (var key in properties) {
			...
			if (properties[key] != null && properties[key].args) this[key].apply(this, properties[key].args);else this[key](properties[key]);
		}
	};
	...
	d2b.UTILS.niceFormat = function (value, precision) {
		...
		if (isNaN(value)) {
			...
			if (absValue < Math.pow(10, 3)) return format(absValue);else if (absValue < Math.pow(10, 6)) return format(absValue / Math.pow(10, 3)) + ' thousand';else if (absValue < Math.pow(10, 9)) return format(absValue / Math.pow(10, 6)) + ' million';else if (absValue < Math.pow(10, 12)) return format(absValue / Math.pow(10, 9)) + ' billion';else if (absValue < Math.pow(10, 15)) return format(absValue / Math.pow(10, 12)) + ' trillion';else if (absValue < Math.pow(10, 18)) return format(absValue / Math.pow(10, 15)) + ' quadrillion';else if (absValue < Math.pow(10, 21)) return format(absValue / Math.pow(10, 18)) + ' quintillion';else if (absValue < Math.pow(10, 24)) return format(absValue / Math.pow(10, 21)) + ' sextillion';else if (absValue < Math.pow(10, 27)) return format(absValue / Math.pow(10, 24)) + ' septillion';else if (absValue < Math.pow(10, 30)) return format(absValue / Math.pow(10, 27)) + ' octillion';else if (absValue < Math.pow(10, 33)) return format(absValue / Math.pow(10, 30)) + ' nonillion';else if (absValue < Math.pow(10, 36)) return format(absValue / Math.pow(10, 33)) + ' decillion';else if (absValue < Math.pow(10, 39)) return format(absValue / Math.pow(10, 36)) + ' undecillion';else if (absValue < Math.pow(10, 42)) return format(absValue / Math.pow(10, 39)) + ' duodecillion';else if (absValue < Math.pow(10, 45)) return format(absValue / Math.pow(10, 42)) + ' tredecillion';else if (absValue < Math.pow(10, 48)) return format(absValue / Math.pow(10, 45)) + ' quattuordecillion';else if (absValue < Math.pow(10, 51)) return format(absValue / Math.pow(10, 48)) + ' quindecillion';else if (absValue < Math.pow(10, 54)) return format(absValue / Math.pow(10, 51)) + ' sexdecillion';else if (absValue < Math.pow(10, 57)) return format(absValue / Math.pow(10, 54)) + ' septendecillion';else if (absValue < Math.pow(10, 60)) return format(absValue / Math.pow(10, 57)) + ' octdecillion';else if (absValue < Math.pow(10, 63)) return format(absValue / Math.pow(10, 60)) + ' novemdecillion';else if (absValue < Math.pow(10, 303)) return format(absValue / Math.pow(10, 63)) + ' vigintillion';else return format(absValue / Math.pow(10, 303)) + ' centillion';
		}
	};
	...
	d2b.UTILS.numberFormat = function (preferences) {
		...
		if (preferences.nice) {
			return function (value) {
				return units.before + d2b.UTILS.niceFormat(value, preferences.precision) + units.after;
			};
		...
		return function (value) {
			if (isNaN(value)) return units.before + value + units.after;else return units.before + format(value) + units.after;
		};
	};
	...
	d2b.UTILS.LAYOUTS.partition = function () {
		...
		$$.group = function (rows, columns) {
			...
			groups.forEach(function (group) {
				...
				if (columns.length) {
					...
					child.size = d3.sum(child.allChildren, function (d) {
						return d.count;
					});
				}

				if (["null", "undefined"].indexOf(child.name) === -1) children.push(child);else {
		...
		var my = function my(data, columns, title) {
			return {
				"name": title,
				"top": true,
	...
	d2b.UTILS.LEGENDS.legend = function () {
		...
		$$.getPathForeground = function (d) {
			...
			if (d.open) return d.path.size(scale * 4)(d);else return d.path.size(scale * 15)(d);
		};
		$$.getPathBackground = function (d) {
			var scale = Math.pow($$.scale / 1.7, 1.5);
			if (d.open || !d.hovered) return d.path.size(scale * 15)(d);else if (d.hovered) return d.path.size(scale * 35)(d);
		};
		...
		$$.enterElements = function () {
			...
			$$.selection.item = $$.selection.selectAll('g.d2b-legend-item').data($$.currentLegendData.items, function (d) {
				return d.label;
			});

			var newItem = $$.selection.item.enter().append('g').attr('class', 'd2b-legend-item').style('opacity', 0).on('mouseover.d2b-mouseover', function (d, i) {
				d.hovered = true;
				var elem = d3.select(this);
				if ($$.active) {
					if (d.open) {
						elem.select('path.d2b-legend-symbol-foreground').transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).style('fill-opacity', 0.4);
					} else {
						elem.select('path.d2b-legend-symbol-background').transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).attr('d', $$.getPathBackground(d));
					}
				}
				...
				elem.select('path.d2b-legend-symbol-foreground').transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).style('fill-opacity', function (d) {
					if (d.open) return 0;else return 0.8;
				});
				elem.select('path.d2b-legend-symbol-background').transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).attr('d', $$.getPathBackground(d));
			}).call($$.events.addElementDispatcher, 'main', 'd2b-legend-item');

			newItem.append('path').attr('class', 'd2b-legend-symbol-background').style('stroke', d2b.UTILS.getColor($$.color, 'label'));

			newItem.append('path').attr('class', 'd2b-legend-symbol-foreground').style('fill', d2b.UTILS.getColor($$.color, 'label'));

			newItem.append('text').text(function (d) {
				return d.label;
			});
		};
		...
		$$.updateElements = function () {
			...
			symbolBackground.transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).style('stroke', d2b.UTILS.getColor($$.color, 'label')).style('stroke-width', 1).attr('transform', 'translate(0,' + $$.scale / 10 + ')').attr('d', function (d) {
				return $$.getPathBackground(d);
			});

			var symbolForeground = $$.selection.item.select('path.d2b-legend-symbol-foreground');

			symbolForeground.transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).attr('transform', 'translate(0,' + $$.scale / 10 + ')').style('fill', d2b.UTILS.getColor($$.color, 'label')).style('fill-opacity', function (d) {
				if (d.open) return d.hovered ? 0.4 : 0;else return 0.8;
			}).attr('d', function (d) {
				return $$.getPathForeground(d);
			});

			$$.selection.item.classed('d2b-active', $$.active ? true : false);

			$$.selection.item.text = $$.selection.item.select('text').style('font-size', $$.scale * 2.5 + 'px').attr('x', $$.scale * 2).attr('y', $$.scale);
		};
		...
		$$.updatePositions = function () {
			...
			if ($$.currentLegendData.items.length > 0) {
				...
				if ($$.orientation == 'vertical') {
					...
					$$.selection.item.transition().duration($$.animationDuration).style('opacity', 1).attr('transform', function (d, i) {
					...
					$$.computedWidth = $$.selection.item.size() > itemsPerRow ? maxItemLength * itemsPerRow - 3 * $$.scale + $$.padding * 2 : maxItemLength * $$.selection.item.size() - 3 * $$.scale + $$.padding * 2;
				}
			}
		};

		$$.exitElements = function () {
			$$.selection.item.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
		};

		var legend = {};

		legend.select = d2b.UTILS.CHARTS.MEMBERS.select(legend, $$, function () {
			$$.generateRequired = true;
		});
		legend.selection = d2b.UTILS.CHARTS.MEMBERS.prop(legend, $$, 'selection', function () {
			$$.generateRequired = true;
		});
		...
		legend.update = function (callback) {
			if (!$$.selection) return console.warn('legend was not given a selection');
	...
	d2b.UTILS.bindTooltip = function (element, html, data, fill) {
		if (html) {
			element.on('mouseover.d2b-mouseover-tooltip', function (d, i) {
	...
	d2b.UTILS.unbindTooltip = function (elem) {
		d2b.UTILS.removeTooltip();
		elem.on('mouseover.d2b-mouseover-tooltip', null).on('mouseout.d2b-mouseout-tooltip', null).on('mousemove.d2b-mousemove-tooltip', null);
	};
	d2b.UTILS.createTooltip = function (elem, html, fill) {
		fill = fill || '#555';
		d3.selectAll('.d2b-general-tooltip').remove();

		var body = d3.select('body');
		var tooltip = body.append('div').attr('class', 'd2b-general-tooltip').html(html).style('color', function (d) {
			return d3.hsl(fill).l > 0.75 ? 'black' : 'white';
		}).style('background', fill).style('border-color', d3.rgb(fill).darker(1)).style('opacity', 0);
		...
		elem.on('mousemove.d2b-mousemove-tooltip', function () {
			tooltip.transition().duration(50).ease('linear').call(d2b.UTILS.moveTooltip);
		});
		return tooltip;
	};
	...
	d2b.UTILS.moveTooltip = function (tooltip) {
		...
		var x = positionRight ? scroll.left + d3.event.clientX + offsetX : scroll.left + d3.event.clientX - tooltipBox.width - offsetX;
		...
		tooltip.each(function (d) {
			...
			elem.classed('d2b-general-tooltip-left', !positionRight).classed('d2b-general-tooltip-right', positionRight);

			elemUpdate.style('opacity', 1).style('top', y + 'px').style('left', x + 'px');
		});
	...
	d2b.UTILS.AXISCHART.TYPES.area = function () {
		...
		$$.area = d3.svg.area().x(function (d) {
			...
			return $$.y(d.yComputed + d.y0Computed);
		}).y0(function (d) {
			return $$.y(d.y0Computed);
		});

		$$.point = d2b.SVG.point().size(40).active(true).fill('white');
		...
		$$.updatePoints = function (graphData, graph) {
			...
			$$.foreground.point = graph.selectAll('g.d2b-point').data(function (d) {
				return d.values;
			});

			var newPoint = $$.foreground.point.enter().append('g').attr('class', 'd2b-point').attr('transform', function (d) {
				return 'translate(' + $$.x(d.x) + ',' + $$.y(d.yComputed + d.y0Computed) + ')';
			}).call($$.events.addElementDispatcher, 'main', 'd2b-area-point');

			$$.point.type(function (d, i) {
				return d.symbol || graphData.symbol || 'circle';
			}).stroke(d2b.UTILS.getColor($$.color, 'label', [graphData]));
			$$.foreground.point.call($$.point);

			$$.foreground.point.call($$.tooltipSVG.graph, {
				key: graphData.label,
				content: $$.tooltip,
				data: function data(d) {
					return { data: d, graph: graphData };
				},
				x: function x(d) {
					return d.x;
				},
				y: function y(d) {
					return d.yComputed + d.y0Computed;
				},
			...
			$$.foreground.point.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
		};

		$$.stack = function (data) {
			if ($$.controlsData.stackAreas.enabled) {
				d3.layout.stack().out(function (d, y0, y) {
					d.y0Computed = y0;
					d.yComputed = y;
				}).offset($$.offset || $$.controlsData.stackAreas.offset || "zero").values(function (d) {
					return d.values;
				})(data);
			} else {
				$$.initYComputed(data);
		...
		chart.xValues = function () {
			...
			$$.currentChartData.forEach(function (graphData) {
				values = values.concat(graphData.values.map(function (d) {
					return d.x;
				}));
			});
			return values;
		...
		chart.update = function (callback) {
			...
			$$.background.each(function (graphData) {
				...
				if (path.size() == 0) {
					path = graph.append('path').call($$.events.addElementDispatcher, 'main', 'd2b-area');
				}

				$$.area.interpolate(graphData.interpolate || 'linear').tension(graphData.tension || 0);

				path.datum(function (d) {
					return graphData.values;
				}).style('fill', d2b.UTILS.getColor($$.color, 'label', [graphData])).style('stroke', d2b.UTILS.getColor($$.color, 'label', [graphData])).transition().duration($$.animationDuration).attr("d", $$.area);
			});
	...
	d2b.UTILS.AXISCHART.TYPES.bar = function () {
		...
		$$.getLinearRangeBand = function () {
			var allXVals = $$.currentChartData.map(function (d) {
		...
		chart.xValues = function () {
			var values = [];
			$$.currentChartData.forEach(function (d) {
				values = values.concat(d.values.map(function (v) {
					return v.x;
				}));
			});
			return values;
		...
		chart.yValues = function () {
			...
			if ($$.controlsData.stackBars.enabled) {
				d3.layout.stack().values(function (d) {
					return d.values;
				}).out($$.buildOut($$.currentChartData.length))($$.currentChartData);
			} else {
				$$.resetY0();
			}
		...
		chart.update = function (callback) {
			...
			var barLabels = $$.currentChartData.filter(function (d) {
				return !d.center;
			}).map(function (d) {
				return d.label;
			});

			$$.groupScale.domain(barLabels).rangeBands([0 + padding, rangeBand - padding]);
			...
			$$.background.each(function (graphData) {
				...
				var newBar = bar.enter().append('rect').style('fill', d2b.UTILS.getColor($$.color, 'label', [graphData])).call($$.events.addElementDispatcher, 'main', 'd2b-bar');
				...
				newBar.attr('y', $$.y(0)).attr('height', 0);

				bar.call($$.tooltipSVG.graph, {
					key: graphData.label,
					content: $$.tooltip,
					data: function data(d) {
						return { data: d, graph: graphData };
					...
					x: function x(d) {
						return $$.x.invert(d.placement.x + d.placement.dx / 2);
					},
					y: function y(d) {
						return d.y + d.y0;
					},
					fill: d2b.UTILS.getColor($$.color, 'label', [graphData])
				}).transition().duration($$.animationDuration).style('fill', d2b.UTILS.getColor($$.color, 'label', [graphData])).attr('x', function (d) {
					return d.placement.x;
				}).attr('y', function (d) {
					return d.placement.y;
				}).attr('width', function (d) {
					return d.placement.dx;
				}).attr('height', function (d) {
					return d.placement.dy;
				});

				bar.exit().transition().duration($$.animationDuration).attr('y', $$.y(0)).attr('height', 0);
			});
	...
	d2b.UTILS.AXISCHART.TYPES.bubblePack = function () {
		...
		$$.tooltip = function (d) {
			return "" + $$.xFormat(d.data.xComputed) + " : " + $$.yFormat(d.data.yComputed) + " (" + d.data.sizeComputed + ")";
		};
		...
		$$.breadcrumbs.set = function (sequence) {
			...
			var breadcrumbsSelection = $$.breadcrumbs.selection();
			breadcrumbsSelection.breadcrumb.path.attr('stroke-width', 1.5).style('fill-opacity', 0.2).style('stroke-opacity', 0.7).style('fill', function (d) {
				return $$.node.fill(d.data);
			}).attr('stroke', function (d) {
				return $$.node.fillDark(d.data);
			});
		};
		...
		$$.point = d2b.SVG.point().type($$.node.symbol).size(function (d) {
			return $$.size(d.sizeComputed);
		}).active(function (d) {
			return d.children && d.children.length;
		}).fill($$.node.fill).stroke($$.node.fillDark).strokeWidth(function (d) {
			return d.children && d.children.length ? '1.5px' : '0px';
		});
		...
		$$.bubble.enterTransform = function (d) {
			if (d.parent) return d.parent.bubble.attr('transform');else return $$.bubble.transform(d);
		};
		...
		$$.bubble.updateChildren = function (d) {
			...
			if ($$.expandedNodes[d.key] && d.children && d.children.length) {
				...
				transition.selectAll('.d2b-bubble-group').style('opacity', 0).remove().selectAll('.d2b-bubble').attr('transform', $$.bubble.transform(d));
			}
		};

		//	Update Bubble Group.
		$$.bubble.update = function (bubbleContainer, bubbleData) {
			var bubbleGroup = bubbleContainer.selectAll('g.d2b-bubble-group').data(bubbleData, $$.bubbleKey);
			var newBubbleGroup = bubbleGroup.enter().append('g').attr('class', 'd2b-bubble-group');

			newBubbleGroup.append('g').call($$.events.addElementDispatcher, 'main', 'd2b-bubble').attr('class', 'd2b-bubble').attr('transform', $$.bubble.enterTransform).on('mouseover.d2b-node-mouseover', $$.node.mouseover).on('mouseout.d2b-node-mouseout', $$.node.mouseout);

			var bubble = bubbleGroup.select('.d2b-bubble').each(function (d) {
				//	Save element to the data.
				d.bubble = d3.select(this);
			}).each($$.bubble.bindClick).transition().duration($$.animationDuration).call($$.point).attr('transform', $$.bubble.transform).each($$.bubble.visibility);

			bubbleGroup.transition().duration($$.animationDuration).each($$.bubble.updateChildren);
		};

		// Define Expanded Bubble Indicator Object
		$$.expandedBubbleIndicator = {};
		$$.expandedBubbleIndicator.enter = function (indicator) {
			var newIndicator = indicator.enter().append('g').attr('class', 'd2b-expanded-bubble-indicator').on('click.d2b-click', $$.node.toggleExpanded).on('mouseover.d2b-node-mouseover', $$.node.mouseover).on('mouseout.d2b-mouseout', $$.node.mouseout).call($$.events.addElementDispatcher, 'main', 'd2b-expanded-bubble-indicator');
			newIndicator.append('rect').style('fill', $$.node.fill).style('stroke', $$.node.fillDark).attr('height', 20);
			newIndicator.append('path').style('fill', $$.node.fill).attr('transform', 'translate(10, 10)');
			newIndicator.append('text').attr('x', 20).attr('y', 15).text(function (d) {
				return d.name.substring(0, 4).trim();
			});
		};
		$$.expandedBubbleIndicator.update = function (indicator, position) {
			var indicatorTransition = indicator.attr('transform', function (d) {
				var elem = d3.select(this);
				var width = elem.select('text').node().getComputedTextLength() + 30;

				//save desired indicatorWidth onto the rect element
				elem.select('rect').each(function () {
			...
			indicatorTransition.select('rect').attr('width', function () {
				return this.indicatorWidth;
			}).style('fill', $$.node.fill).style('stroke', $$.node.fillDark);
			indicatorTransition.select('path').attr('d', $$.node.getSymbolPathSmall).style('fill', $$.node.fill);

			$$.breadcrumbsContainer.attr('transform', 'translate(' + 10 + ',' + (position.y + 30) + ')');
		};
		$$.expandedBubbleIndicator.exit = function (indicator) {
			indicator.exit().transition().duration(d2b.CONSTANTS.ANIMATIONLENGTHS().short).style('opacity', 0).remove();
		};
		...
		$$.cleanSizeScale = function (scale) {
			if (scale) {
				if (scale.length === 1) return [scale[0], scale[0]];else return scale;
			} else if (scale === null) {
				return [1, 1];
			}
		};
		...
		$$.formatData = function () {
			...
			$$.currentChartData.forEach(function (graphData, graphIndex) {
				$$.traverseTree(graphData.pack, function (parent) {
					...
					this.sizeComputed = d3.sum(this.values, function (d) {
						return d.size || 1;
					});
					this.xComputed = d2b.MATH[$$.positionTypeX](this.values, function (d) {
						return d.x || 0;
					}, function (d) {
						return d.size || 1;
					});
					this.yComputed = d2b.MATH[$$.positionTypeY](this.values, function (d) {
						return d.y || 0;
					}, function (d) {
						return d.size || 1;
					});
		...
		chart.update = function (callback) {
			...
			$$.size.domain([0, d3.max($$.currentChartData, function (d) {
				return d.pack.sizeComputed;
			})]);
   	...
   	*/
			...
			if (!$$.breadcrumbsContainer.size()) {
				$$.breadcrumbsContainer = $$.general.append('g').attr('class', 'd2b-breadcrumbs');
			...
			$$.foreground.each(function (graphData) {
				...
				var indicator = graph.selectAll('g.d2b-expanded-bubble-indicator').data(sorting.expanded, function (d) {
					return d.key;
				});
				...
				indicator.call(d2b.UTILS.bindTooltip, $$.tooltip, function (d) {
					return { data: d, graph: graphData };
				}, $$.node.fill);
			});
	...
	d2b.UTILS.AXISCHART.TYPES.gridMarker = function () {
		...
		$$.newGroup = function (elem, graphData) {
			var newXMarker = elem.append('g').attr('class', 'd2b-grid-marker-x').call($$.updateMarkers, graphData, 'x', false);

			newXMarker.append('line').attr('class', 'd2b-grid-marker-line');
			newXMarker.append('text').attr('class', 'd2b-grid-marker-coordinate').attr('transform', 'rotate(-90)').attr('y', 7 + $$.labelOffset);

			var newYMarker = elem.append('g').attr('class', 'd2b-grid-marker-y').call($$.updateMarkers, graphData, 'y', false);

			newYMarker.append('line').attr('class', 'd2b-grid-marker-line');
			newYMarker.append('text').attr('class', 'd2b-grid-marker-coordinate').attr('y', 7 + $$.labelOffset);

			var newGroupCenter = elem.append('g').attr('class', 'd2b-grid-marker-center').call($$.updateCenter, graphData, false).call($$.events.addElementDispatcher, 'main', 'd2b-grid-marker');

			elem.append('g').attr('class', 'd2b-grid-marker-label').call($$.updateLabel, graphData, false).append('text');
		};

		$$.updateMarker = {};
		$$.updateMarker.x = function (elem, range, graphData) {
			elem.style('opacity', 1).attr('transform', function (d) {
				return 'translate(' + $$.x(d.x) + ',0)';
			}).select('line').attr('x1', 0).attr('x2', 0).attr('y1', range[0]).attr('y2', range[1]).style('stroke', d2b.UTILS.getColor($$.color, 'label', [graphData]));

			elem.select('.d2b-grid-marker-coordinate').attr('x', function (d) {
				return d.invert ? -range[0] + $$.labelOffset : -$$.labelOffset;
			});
		};
		$$.updateMarker.y = function (elem, range, graphData) {
			elem.style('opacity', 1).attr('transform', function (d) {
				return 'translate(' + range[1] + ',' + $$.y(d.y) + ')';
			}).select('line').attr('x1', range[0]).attr('x2', -range[1]).attr('y1', 0).attr('y2', 0).style('stroke', d2b.UTILS.getColor($$.color, 'label', [graphData]));

			elem.select('.d2b-grid-marker-coordinate').attr('x', function (d) {
				return d.invert ? -range[1] + $$.labelOffset : -$$.labelOffset;
			});
		};
		...
		$$.updateMarkers = function (elem, graphData, orient, transition) {
			...
			elem.each(function (d) {
				...
				if (transition) elemTransition = elemTransition.transition().duration($$.animationDuration);

				if (d[x]) {
					elemTransition.call($$.updateMarker[x], range, graphData);
				} else {
					elemTransition.style('opacity', 0);
				}
			});

			//set line attributes
			elem.select('line').classed('d2b-dashed', function (d) {
				return d.dashed;
			});

			elem.select('text.d2b-grid-marker-coordinate').classed('d2b-inverted', function (d) {
				return d.invert;
			}).text(function (d) {
				return $$[x + 'Format'](d[x]);
			});
		...
		$$.updateLabel = function (elem, graphData, transition) {
			...
			this.each(function (d) {
				...
				textTransition.text(function (d) {
					return d.label;
				}).attr('y', -$$.labelOffset).attr('transform', 'rotate(0)');

				if (d.x != null && d.y != null) {
					if ($$.x(d.x) > $$.width / 2) {
						elemTransition.attr('transform', 'translate(' + $$.x(d.x) + ',' + $$.y(d.y) + ')');

						textTransition.style('text-anchor', 'end').attr('x', $$.labelOffset);
					} else {
						;
						elemTransition.attr('transform', 'translate(' + $$.x(d.x) + ',' + $$.y(d.y) + ')');

						textTransition.style('text-anchor', 'start').attr('x', $$.labelOffset);
					}
				} else if (d.y != null) {
					if (d.invert) {
						elemTransition.attr('transform', 'translate(' + xRange[0] + ',' + $$.y(d.y) + ')');
						textTransition.style('text-anchor', 'start').attr('x', $$.labelOffset);
					} else {
						elemTransition.attr('transform', 'translate(' + xRange[1] + ',' + $$.y(d.y) + ')');
						textTransition.style('text-anchor', 'end').attr('x', -$$.labelOffset);
					}
				} else {
					if (d.invert) {
						elemTransition.attr('transform', 'translate(' + $$.x(d.x) + ',' + yRange[0] + ')');
						textTransition.style('text-anchor', 'start').attr('x', $$.labelOffset).attr('transform', 'rotate(-90)');
					} else {
						elemTransition.attr('transform', 'translate(' + $$.x(d.x) + ',' + yRange[1] + ')');
						textTransition.style('text-anchor', 'end').attr('x', -$$.labelOffset).attr('transform', 'rotate(-90)');
					}
				}
			});
		};
		...
		$$.updateCenter = function (elem, graphData, transition) {
			...
			elem.each(function (d) {
				...
				if (transition) elem = elem.transition().duration($$.animationDuration);
				...
				if (d.y != null && d.x != null) {
					...
					elem.style('opacity', 1).call($$.point).attr('transform', 'translate(' + $$.x(d.x) + ',' + $$.y(d.y) + ')');
				} else {
			...
			$$.currentChartData.forEach(function (d) {
				if (d.values) {
					d.values.forEach(function (d2) {
						if (d2.x) values.push(d2.x);
					});
				} else if (d.x) values.push(d.x);
			});
			return values;
		};
		chart.yValues = function () {
			...
			$$.currentChartData.forEach(function (d) {
				if (d.values) {
					d.values.forEach(function (d2) {
						if (d2.y) values.push(d2.y);
					});
				} else if (d.y) values.push(d.y);
			});
			return values;
		};
		...
		chart.update = function (callback) {
			...
			$$.foreground.each(function (graphData) {
				...
				var newGroup = group.enter().append('g').attr('class', 'd2b-grid-marker').call($$.newGroup, graphData);

				group.select('g.d2b-grid-marker-x').call($$.updateMarkers, graphData, 'x', true);

				group.select('g.d2b-grid-marker-y').call($$.updateMarkers, graphData, 'y', true);

				group.select('g.d2b-grid-marker-label').call($$.updateLabel, graphData, 'y', true);
				...
				groupCenter.call($$.updateCenter, graphData, true);

				//exit group

				group.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
			});
	...
	d2b.UTILS.AXISCHART.TYPES.heatPoints = function () {
		...
		$$.renderLayout.square = function (points) {
			...
			points.forEach(function (point, i) {
				...
				elem.transition().duration($$.animationDuration / 2).delay(delay * i).attr("d", $$.symbol);
		...
		chart.update = function (callback) {
			...
			$$.background.each(function (graphData, i) {
				...
				heatPointSet.enter().append('g').attr('class', 'd2b-heat-point-set').attr('transform', function (d) {
					return 'translate(' + $$.x(d.x) + ',' + $$.y(d.y) + ')';
				});

				heatPointSet.transition().duration($$.animationDuration).attr('transform', function (d) {
					return 'translate(' + $$.x(d.x) + ',' + $$.y(d.y) + ')';
				}).style('opacity', 1).each(function (d) {
					var key = $$.key(d);
					if (mergedPoints[key]) {
				...
				heatPointSet.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();

				var heatPoint = heatPointSet.selectAll('.d2b-heat-point').data(function (d) {
					return d.points;
				});

				heatPoint.enter().append('path').attr('d', $$.symbol.size(0)).attr('class', 'd2b-heat-point');

				heatPoint.style('opacity', 1).each(function (d) {
					d.elem = d3.select(this);
				});

				heatPoint.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
			});

			var space = Math.min($$.width / Object.keys(xKeys).length, $$.height / Object.keys(yKeys).length);

			if ($$.size) {
				$$.symbolSize = $$.size;
			} else {
				$$.symbolSize = Math.pow(space / Math.sqrt(maxPoints), 2);
				if ($$.layout == 'circle') $$.symbolSize *= 0.3;else $$.symbolSize *= 0.5;
			}
			...
			$$.background.selectAll(".d2b-heat-point").style('fill', d2b.UTILS.getColor($$.color, 'name')).style('stroke', d2b.UTILS.getColor($$.color, 'name'));
	...
	d2b.UTILS.AXISCHART.TYPES.histogram = function () {
		...
		chart.xValues = function () {
			var values = [];
			$$.currentChartData.forEach(function (graphData) {
				values = values.concat(graphData.histData.map(function (d) {
					return d.x;
				}));
			});
			return values;
		...
		chart.update = function (callback) {
			...
			$$.background.each(function (graphData) {
				...
				var xVals = graphData.histData.map(function (d) {
					return d.x;
				});
				var xRange = [$$.x(d3.min(xVals)), $$.x(d3.max(xVals))];
				...
				var newBar = bar.enter().append('rect').style('fill', d2b.UTILS.getColor($$.color, 'label', [graphData])).attr('y', $$.y(0)).attr('height', 0).call($$.events.addElementDispatcher, 'main', 'd2b-histogram-bar');
				...
				.call($$.tooltipSVG.graph, {
					...
					x: function x(d) {
						return $$.x.invert($$.x(d.x) - padding + barWidth / 2);
					},
					y: function y(d) {
						return d.y;
					},
	...
	d2b.UTILS.AXISCHART.TYPES.line = function () {
		...
		chart.update = function (callback) {
			$$.background.each(function (graphData, i) {
				...
				var path = graph.select('path');
				if (path.size() === 0) {
					path = graph.append('path').call($$.events.addElementDispatcher, 'main', 'd2b-line');
				}

				$$.line.interpolate(graphData.interpolate || 'linear').tension(graphData.tension || 0);

				path.style('stroke', d2b.UTILS.getColor($$.color, 'label', [graphData])).datum(function (d) {
					return graphData.values;
				}).transition().duration($$.animationDuration).attr("d", $$.line);
			});

			$$.foreground.each(function (graphData) {
				var graph = d3.select(this);
				var point = graph.selectAll('g').data(function (d) {
					return d.values;
				});

				var newPoint = point.enter().append('g').attr('transform', function (d) {
					return 'translate(' + $$.x(d.x) + ',' + $$.y(d.y) + ')';
				});
				...
				point.call($$.point).call($$.events.addElementDispatcher, 'main', 'd2b-line-point').call($$.tooltipSVG.graph, {
					key: graphData.label,
					content: $$.tooltip,
					data: function data(d) {
						return { data: d, graph: graphData };
				...
				point.transition().duration($$.animationDuration).attr('transform', function (d) {
					return 'translate(' + $$.x(d.x) + ',' + $$.y(d.y) + ')';
				});

				point.exit().transition().duration($$.animationDuration).style('opacity', 0).remove();
			});
	...
	d2b.UTILS.AXISCHART.TYPES.scatter = function () {
		...
		$$.point = d2b.SVG.point().size(function (d) {
			return d.size || 40;
		}).active(true);
		...
		chart.update = function (callback) {
			...
			$$.foreground.each(function (graphData) {
				...
				var point = graph.selectAll('g').data(function (d) {
					...
					return d.key || d.x + '-' + d.y;
				});

				var fill = d2b.UTILS.getColor($$.color, 'label', [graphData]);
				$$.point.stroke(fill).fill(fill).type(function (d, i) {
					return d.symbol || graphData.symbol || 'circle';
				});

				var newPoint = point.enter().append('g').call($$.point.size(0)).attr('transform', function (d) {
					return 'translate(' + $$.x(d.x) + ',' + $$.y(d.y) + ')';
				});
				...
				point.call($$.events.addElementDispatcher, 'main', 'd2b-scatter-point').call($$.tooltipSVG.graph, {
					key: graphData.label,
					content: $$.tooltip,
					data: function data(d) {
						return { data: d, graph: graphData };
				...
				point.transition().duration($$.animationDuration).style('opacity', 1).call($$.point).attr('transform', function (d) {
					return 'translate(' + $$.x(d.x) + ',' + $$.y(d.y) + ')';
				});
