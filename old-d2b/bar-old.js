  ...
  const model = base(bar, $$)
      ...
      .addMethod('setVisiblePoints', context => {
        ...
        selection.each(function (d, i) {
          ...
          let orient = {};
          if ($$.orient.call(this, d, i) === 'horizontal') {
            orient = { px: 'py', py: 'px', x: 'y', y: 'x' };
          } else {
            orient = { px: 'px', py: 'py', x: 'x', y: 'y' };
          }
          ...
          graphs.forEach(graph => {
            ...
            graph.__visiblePoints__ = values.map(v => {
              const point = {};
              point[orient.x] = v.__base__;
              point[orient.y] = v.__extent__[0];
              return point;
            })
            .concat(values.map(v => {
              const point = {};
              point[orient.x] = v.__base__;
              point[orient.y] = v.__extent__[1];
              return point;
            }));
          });
