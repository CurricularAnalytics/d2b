...
export default function () {
  ...
  const bubblePack = function (context) {
    const transition = context.selection? context : null,
          ...
          graph = selection.selectAll('.d2b-bubble-pack-graph').data(d => $$.graphs(d), $$.key);
    ...
    context.each(function (d, i) {
      ...
      graph.each( function (d, i) {
        ...
        renderPacks(el, values, transition, x, y, shift, selection, addTooltipPoint);
      });
  ...
  const model = base(bubblePack, $$)
    ...
    .addMethod('setVisiblePoints', context => {
      ...
      selection.each(function (d, i) {
        $$.graphs(d).forEach( graph => {

          const values = $$.values(graph);
          graph.__visiblePoints__ = [];

          values.forEach(compute);
          values.forEach(addPoint);

          function addPoint(v) {
            if (!v.__expanded__) {
              graph.__visiblePoints__.push({x: v.__x__, y: v.__y__});
            } else {
              ($$.pchildren(v) || []).forEach(addPoint);
            }
          }
        });
      });
